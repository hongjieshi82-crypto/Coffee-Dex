import { buildQualityControlledToxicQuotes } from "@/toxic-quote-pool";

export interface CoffeeType {
  id: string;
  categoryId: string;
  name: string;
  en: string;
  gradient: [string, string];
}

export interface CoffeeCategory {
  id: string;
  name: string;
  en: string;
  items: CoffeeType[];
}

export const CURRENT_STICKER_VERSION = 4;
const MIN_USABLE_STICKER_DATA_URL_LENGTH = 1_000;

export interface CoffeeRecord {
  id: string;
  coffeeType: string;
  coffeeName: string;
  categoryId: string;
  volumeMl: number;
  imageData?: string;
  stickerData?: string;
  stickerVersion?: number;
  caffeine: number;
  temp?: string | null;
  sugar?: string | null;
  aiComment: string;
  toxicQuote: string;
  timestamp: number;
}

export function hasUsableStickerData(record: CoffeeRecord) {
  const stickerData = record.stickerData;
  if (!stickerData) return false;

  if (/^https:\/\//i.test(stickerData)) return true;

  return stickerData.startsWith("data:image/png;base64,") &&
    stickerData.length >= MIN_USABLE_STICKER_DATA_URL_LENGTH;
}

export function hasUsableCurrentSticker(record: CoffeeRecord) {
  return hasUsableStickerData(record) &&
    (record.stickerVersion ?? 0) >= CURRENT_STICKER_VERSION;
}

const withCategory = (
  categoryId: string,
  items: Array<Omit<CoffeeType, "categoryId">>
): CoffeeType[] => items.map((item) => ({ ...item, categoryId }));

export const coffeeCategories: CoffeeCategory[] = [
  {
    id: "espresso-base",
    name: "意式经典",
    en: "Espresso Base",
    items: withCategory("espresso-base", [
      { id: "espresso", name: "Espresso", en: "Espresso", gradient: ["#292524", "#0a0a0a"] },
      { id: "americano", name: "美式", en: "Americano", gradient: ["#78350f", "#1c1917"] },
      { id: "latte", name: "拿铁", en: "Latte", gradient: ["#fbbf24", "#d97706"] },
      { id: "cappuccino", name: "卡布奇诺", en: "Cappuccino", gradient: ["#fdba74", "#92400e"] },
      { id: "flat-white", name: "澳白", en: "Flat White", gradient: ["#9a3412", "#451a03"] },
      { id: "cortado", name: "可塔朵", en: "Cortado", gradient: ["#a16207", "#78350f"] },
      { id: "macchiato", name: "玛奇朵", en: "Macchiato", gradient: ["#d4a373", "#6b4226"] },
      { id: "ristretto", name: "精萃", en: "Ristretto", gradient: ["#1c1917", "#0a0a0a"] },
      { id: "long-black", name: "长黑", en: "Long Black", gradient: ["#451a03", "#0a0a0a"] },
      { id: "lungo", name: "加长浓缩", en: "Lungo", gradient: ["#57534e", "#1c1917"] },
      { id: "piccolo", name: "短笛咖啡", en: "Piccolo", gradient: ["#d97706", "#78350f"] },
      { id: "breve", name: "布雷卫", en: "Breve", gradient: ["#f5d0a9", "#92400e"] },
    ]),
  },
  {
    id: "milk-sweet",
    name: "奶咖甜咖",
    en: "Milk & Sweet",
    items: withCategory("milk-sweet", [
      { id: "vanilla-latte", name: "香草拿铁", en: "Vanilla Latte", gradient: ["#fef3c7", "#d97706"] },
      { id: "caramel-macchiato", name: "焦糖玛奇朵", en: "Caramel Macchiato", gradient: ["#fbbf24", "#92400e"] },
      { id: "mocha", name: "摩卡", en: "Mocha", gradient: ["#881337", "#78350f"] },
      { id: "hazelnut-latte", name: "榛果拿铁", en: "Hazelnut Latte", gradient: ["#a16207", "#78350f"] },
      { id: "affogato", name: "阿芙佳朵", en: "Affogato", gradient: ["#fde68a", "#92400e"] },
      { id: "osmanthus-latte", name: "桂花拿铁", en: "Osmanthus Latte", gradient: ["#fbbf24", "#b45309"] },
      { id: "coconut-latte", name: "生椰拿铁", en: "Coconut Latte", gradient: ["#e2e8f0", "#94a3b8"] },
      { id: "spanish-latte", name: "西班牙拿铁", en: "Spanish Latte", gradient: ["#f59e0b", "#7c2d12"] },
      { id: "sea-salt-latte", name: "海盐拿铁", en: "Sea Salt Latte", gradient: ["#93c5fd", "#92400e"] },
      { id: "pistachio-latte", name: "开心果拿铁", en: "Pistachio Latte", gradient: ["#84cc16", "#3f6212"] },
      { id: "rose-latte", name: "玫瑰拿铁", en: "Rose Latte", gradient: ["#fb7185", "#9f1239"] },
      { id: "toffee-nut-latte", name: "太妃榛果拿铁", en: "Toffee Nut Latte", gradient: ["#fbbf24", "#78350f"] },
    ]),
  },
  {
    id: "filter",
    name: "手冲精品",
    en: "Filter Coffee",
    items: withCategory("filter", [
      { id: "yirgacheffe", name: "耶加雪菲", en: "Yirgacheffe", gradient: ["#581c87", "#312e81"] },
      { id: "colombia", name: "哥伦比亚", en: "Colombia", gradient: ["#78350f", "#451a03"] },
      { id: "kenya", name: "肯尼亚", en: "Kenya", gradient: ["#991b1b", "#450a0a"] },
      { id: "panama-geisha", name: "巴拿马瑰夏", en: "Panama Geisha", gradient: ["#b45309", "#78350f"] },
      { id: "mandheling", name: "曼特宁", en: "Mandheling", gradient: ["#365314", "#1a2e05"] },
      { id: "brazil", name: "巴西", en: "Brazil", gradient: ["#166534", "#052e16"] },
      { id: "ethiopia", name: "埃塞俄比亚", en: "Ethiopia", gradient: ["#7c2d12", "#312e81"] },
      { id: "guatemala", name: "危地马拉", en: "Guatemala", gradient: ["#854d0e", "#365314"] },
      { id: "costa-rica", name: "哥斯达黎加", en: "Costa Rica", gradient: ["#b91c1c", "#0f766e"] },
      { id: "honduras", name: "洪都拉斯", en: "Honduras", gradient: ["#1d4ed8", "#451a03"] },
      { id: "rwanda", name: "卢旺达", en: "Rwanda", gradient: ["#be123c", "#4c1d95"] },
    ]),
  },
  {
    id: "cold",
    name: "冷萃冰咖",
    en: "Cold Coffee",
    items: withCategory("cold", [
      { id: "iced-americano", name: "冰美式", en: "Iced Americano", gradient: ["#1e3a5f", "#0c4a6e"] },
      { id: "cold-brew", name: "冷萃", en: "Cold Brew", gradient: ["#0c4a6e", "#451a03"] },
      { id: "nitro-coffee", name: "氮气咖啡", en: "Nitro Coffee", gradient: ["#374151", "#111827"] },
      { id: "iced-latte", name: "冰拿铁", en: "Iced Latte", gradient: ["#93c5fd", "#2563eb"] },
      { id: "frappuccino", name: "星冰乐", en: "Frappuccino", gradient: ["#c084fc", "#7c3aed"] },
      { id: "lemon-coffee", name: "冻柠咖啡", en: "Lemon Coffee", gradient: ["#fde047", "#ca8a04"] },
      { id: "iced-oat-latte", name: "冰燕麦拿铁", en: "Iced Oat Latte", gradient: ["#bae6fd", "#a16207"] },
      { id: "iced-spanish-latte", name: "冰西班牙拿铁", en: "Iced Spanish Latte", gradient: ["#38bdf8", "#b45309"] },
      { id: "cold-brew-latte", name: "冷萃拿铁", en: "Cold Brew Latte", gradient: ["#0f766e", "#92400e"] },
      { id: "iced-mocha", name: "冰摩卡", en: "Iced Mocha", gradient: ["#60a5fa", "#581c87"] },
      { id: "shakerato", name: "冰摇浓缩", en: "Shakerato", gradient: ["#334155", "#0f172a"] },
    ]),
  },
  {
    id: "signature",
    name: "特调创意",
    en: "Signature & Trend",
    items: withCategory("signature", [
      { id: "dirty", name: "Dirty", en: "Dirty", gradient: ["#44403c", "#171717"] },
      { id: "coconut-coffee", name: "椰子水咖啡", en: "Coconut Coffee", gradient: ["#a3e635", "#65a30d"] },
      { id: "orange-americano", name: "橙子美式", en: "Orange Americano", gradient: ["#f97316", "#c2410c"] },
      { id: "liquor-coffee", name: "酒香咖啡", en: "Liquor Coffee", gradient: ["#92400e", "#451a03"] },
      { id: "oat-latte", name: "燕麦拿铁", en: "Oat Latte", gradient: ["#d4a373", "#a16207"] },
      { id: "sparkling-americano", name: "气泡美式", en: "Sparkling Americano", gradient: ["#67e8f9", "#0891b2"] },
      { id: "plum-americano", name: "话梅美式", en: "Plum Americano", gradient: ["#c084fc", "#7c3aed"] },
      { id: "espresso-tonic", name: "汤力咖啡", en: "Espresso Tonic", gradient: ["#67e8f9", "#451a03"] },
      { id: "yuzu-americano", name: "柚子美式", en: "Yuzu Americano", gradient: ["#facc15", "#0f766e"] },
      { id: "osmanthus-americano", name: "桂花美式", en: "Osmanthus Americano", gradient: ["#fbbf24", "#78350f"] },
      { id: "pineapple-americano", name: "菠萝美式", en: "Pineapple Americano", gradient: ["#fde047", "#16a34a"] },
      { id: "mint-mocha", name: "薄荷摩卡", en: "Mint Mocha", gradient: ["#2dd4bf", "#451a03"] },
    ]),
  },
  {
    id: "milk-tea",
    name: "奶茶鲜奶",
    en: "Milk Tea",
    items: withCategory("milk-tea", [
      { id: "classic-milk-tea", name: "经典奶茶", en: "Classic Milk Tea", gradient: ["#b45309", "#78350f"] },
      { id: "bubble-milk-tea", name: "珍珠奶茶", en: "Bubble Milk Tea", gradient: ["#92400e", "#451a03"] },
      { id: "jasmine-milk-tea", name: "茉莉奶绿", en: "Jasmine Milk Green", gradient: ["#65a30d", "#14532d"] },
      { id: "oolong-milk-tea", name: "乌龙奶茶", en: "Oolong Milk Tea", gradient: ["#a16207", "#3f2a14"] },
      { id: "cheese-tea", name: "芝士茶", en: "Cheese Tea", gradient: ["#facc15", "#b45309"] },
      { id: "brown-sugar-milk", name: "黑糖鲜奶", en: "Brown Sugar Milk", gradient: ["#854d0e", "#1c1917"] },
    ]),
  },
  {
    id: "fruit-tea",
    name: "果茶清爽",
    en: "Fruit Tea",
    items: withCategory("fruit-tea", [
      { id: "fresh-lemon-tea", name: "鲜切柠檬茶", en: "Fresh Lemon Tea", gradient: ["#facc15", "#65a30d"] },
      { id: "grape-lemon-tea", name: "葡萄鲜切柠檬茶", en: "Grape Lemon Tea", gradient: ["#7c3aed", "#84cc16"] },
      { id: "fresh-fruit-tea", name: "满杯水果茶", en: "Fresh Fruit Tea", gradient: ["#f97316", "#dc2626"] },
      { id: "passionfruit-tea", name: "百香果茶", en: "Passion Fruit Tea", gradient: ["#fde047", "#f97316"] },
      { id: "peach-oolong", name: "白桃乌龙", en: "Peach Oolong", gradient: ["#fb7185", "#a16207"] },
      { id: "cheese-grape", name: "芝士葡萄", en: "Cheese Grape", gradient: ["#9333ea", "#facc15"] },
    ]),
  },
  {
    id: "alt-drinks",
    name: "非咖啡替代饮",
    en: "Coffee Alternatives",
    items: withCategory("alt-drinks", [
      { id: "matcha-latte", name: "抹茶拿铁", en: "Matcha Latte", gradient: ["#22c55e", "#166534"] },
      { id: "cocoa", name: "可可", en: "Cocoa", gradient: ["#78350f", "#451a03"] },
      { id: "black-tea-latte", name: "红茶拿铁", en: "Black Tea Latte", gradient: ["#dc2626", "#7f1d1d"] },
      { id: "herbal", name: "草本饮品", en: "Herbal Drink", gradient: ["#4ade80", "#166534"] },
      { id: "chai-latte", name: "印度奶茶", en: "Chai Latte", gradient: ["#d97706", "#92400e"] },
      { id: "earl-grey", name: "伯爵茶", en: "Earl Grey", gradient: ["#6366f1", "#312e81"] },
    ]),
  },
];

export const coffeeTypes = coffeeCategories.flatMap((category) => category.items);
export const coffeeTypeMap = Object.fromEntries(coffeeTypes.map((coffee) => [coffee.id, coffee]));
export const coffeeCategoryMap = Object.fromEntries(coffeeCategories.map((category) => [category.id, category]));

export interface CoffeeSearchMatch {
  category: CoffeeCategory;
  coffee: CoffeeType;
}

const coffeeAliases: Record<string, string[]> = {
  americano: ["美式咖啡", "黑咖啡", "小黄油美式", "黄油美式", "奶油美式", "青提美式", "茉莉美式"],
  "iced-americano": ["冰美", "冰美式咖啡"],
  "long-black": ["澳式黑咖啡", "长黑咖啡", "longblack"],
  lungo: ["加长萃取", "加长浓缩咖啡"],
  piccolo: ["短笛", "短笛拿铁", "piccolo latte"],
  breve: ["半奶油拿铁", "布雷夫"],
  latte: ["拿铁咖啡", "latte"],
  "flat-white": ["澳瑞白", "馥芮白", "flatwhite"],
  cappuccino: ["卡布", "卡布基诺"],
  macchiato: ["玛琪朵", "玛奇雅朵"],
  "caramel-macchiato": ["焦玛", "焦糖玛琪朵"],
  "spanish-latte": ["西班牙拿铁咖啡", "炼乳拿铁"],
  "sea-salt-latte": ["海盐咖啡", "海盐奶咖"],
  "pistachio-latte": ["开心果咖啡", "开心果奶咖"],
  "rose-latte": ["玫瑰咖啡", "玫瑰奶咖"],
  "toffee-nut-latte": ["太妃拿铁", "太妃榛果", "榛果太妃拿铁"],
  ethiopia: ["埃塞", "埃塞豆", "埃塞俄比亚咖啡"],
  guatemala: ["危地马拉咖啡", "危地马拉豆"],
  "costa-rica": ["哥斯达黎加咖啡", "哥斯达黎加豆"],
  honduras: ["洪都拉斯咖啡", "洪都拉斯豆"],
  rwanda: ["卢旺达咖啡", "卢旺达豆"],
  "cold-brew": ["冷泡", "冷萃咖啡"],
  "iced-oat-latte": ["冰燕麦", "冰燕麦奶拿铁"],
  "iced-spanish-latte": ["冰西班牙", "冰炼乳拿铁"],
  "cold-brew-latte": ["冷萃奶咖", "冷萃拿铁咖啡"],
  "iced-mocha": ["冰摩卡咖啡"],
  shakerato: ["冰摇咖啡", "冰摇浓缩咖啡"],
  dirty: ["脏咖啡", "脏脏"],
  "coconut-latte": ["生椰", "生椰拿铁咖啡"],
  "orange-americano": ["橙美式", "橙c美式"],
  "espresso-tonic": ["咖啡汤力", "浓缩汤力", "气泡汤力咖啡"],
  "yuzu-americano": ["柚子咖啡", "柚子冰美式"],
  "osmanthus-americano": ["桂花咖啡", "桂花冰美式"],
  "pineapple-americano": ["菠萝咖啡", "凤梨美式"],
  "mint-mocha": ["薄荷咖啡", "薄荷巧克力咖啡"],
  "oat-latte": ["燕麦奶拿铁"],
  "matcha-latte": ["抹茶", "抹茶拿铁咖啡"],
  cocoa: ["热可可", "巧克力"],
  "black-tea-latte": ["红茶拿铁", "鸳鸯"],
  "chai-latte": ["印度奶茶", "香料奶茶"],
  "earl-grey": ["伯爵", "伯爵红茶"],
  "classic-milk-tea": ["奶茶", "原味奶茶", "港式奶茶"],
  "bubble-milk-tea": ["珍奶", "波霸奶茶", "珍珠"],
  "jasmine-milk-tea": ["奶绿", "茉莉奶茶", "茉香奶绿"],
  "oolong-milk-tea": ["乌龙拿铁", "乌龙鲜奶茶", "乌龙奶"],
  "cheese-tea": ["奶盖茶", "芝士奶盖", "芝士茗茶"],
  "brown-sugar-milk": ["黑糖珍珠鲜奶", "黑糖牛奶", "黑糖波霸鲜奶"],
  "fresh-lemon-tea": ["柠檬茶", "鲜柠茶", "手打柠檬茶", "鸭屎香柠檬茶", "香水柠檬茶"],
  "grape-lemon-tea": ["葡萄柠檬茶", "葡萄鲜切", "鲜切葡萄柠檬茶", "葡萄鲜切柠檬茶"],
  "fresh-fruit-tea": ["水果茶", "鲜果茶", "满杯水果", "鲜切水果茶"],
  "passionfruit-tea": ["百香果", "百香果柠檬茶", "百香果绿茶"],
  "peach-oolong": ["白桃乌龙茶", "蜜桃乌龙", "桃桃乌龙"],
  "cheese-grape": ["多肉葡萄", "芝士多肉葡萄", "葡萄奶盖"],
};

interface CoffeeFamily {
  keywords: string[];
  ids: string[];
  generic?: boolean;
  scoreBase?: number;
}

const coffeeFamilies: CoffeeFamily[] = [
  {
    keywords: ["美式", "americano", "黑咖啡"],
    ids: [
      "americano",
      "iced-americano",
      "long-black",
      "orange-americano",
      "sparkling-americano",
      "plum-americano",
      "yuzu-americano",
      "osmanthus-americano",
      "pineapple-americano",
    ],
  },
  {
    keywords: ["拿铁", "latte"],
    ids: [
      "latte",
      "vanilla-latte",
      "hazelnut-latte",
      "osmanthus-latte",
      "coconut-latte",
      "iced-latte",
      "spanish-latte",
      "sea-salt-latte",
      "pistachio-latte",
      "rose-latte",
      "toffee-nut-latte",
      "oat-latte",
      "iced-oat-latte",
      "iced-spanish-latte",
      "cold-brew-latte",
      "matcha-latte",
      "black-tea-latte",
      "chai-latte",
    ],
  },
  {
    keywords: ["玛奇朵", "玛琪朵", "macchiato"],
    ids: ["macchiato", "caramel-macchiato"],
  },
  {
    keywords: ["冷萃", "冷泡", "coldbrew"],
    ids: ["cold-brew", "cold-brew-latte", "nitro-coffee"],
  },
  {
    keywords: ["摩卡", "mocha", "巧克力"],
    ids: ["mocha", "iced-mocha", "mint-mocha", "cocoa"],
  },
  {
    keywords: ["椰", "coconut"],
    ids: ["coconut-latte", "coconut-coffee"],
  },
  {
    keywords: ["手冲", "单品", "豆", "瑰夏", "ethiopia", "geisha"],
    ids: ["yirgacheffe", "ethiopia", "panama-geisha", "colombia", "kenya", "guatemala", "costa-rica"],
  },
  {
    keywords: ["抹茶", "matcha"],
    ids: ["matcha-latte"],
  },
  {
    keywords: ["奶茶", "珍珠", "奶绿", "鲜奶", "milk tea"],
    ids: ["classic-milk-tea", "bubble-milk-tea", "jasmine-milk-tea", "oolong-milk-tea", "brown-sugar-milk"],
  },
  {
    keywords: ["柠檬", "鲜切", "lemon"],
    ids: ["fresh-lemon-tea", "grape-lemon-tea", "passionfruit-tea", "fresh-fruit-tea"],
  },
  {
    keywords: ["葡萄", "grape"],
    ids: ["grape-lemon-tea", "cheese-grape", "fresh-fruit-tea"],
  },
  {
    keywords: ["水果", "果茶", "鲜果", "fruit"],
    ids: ["fresh-fruit-tea", "passionfruit-tea", "grape-lemon-tea", "peach-oolong"],
  },
  {
    keywords: ["茶", "tea"],
    ids: [
      "fresh-lemon-tea",
      "fresh-fruit-tea",
      "peach-oolong",
      "earl-grey",
      "jasmine-milk-tea",
      "matcha-latte",
      "black-tea-latte",
      "chai-latte",
      "herbal",
    ],
    generic: true,
    scoreBase: 6,
  },
];

export function searchCoffeeTypes(term: string, limit = 8): CoffeeSearchMatch[] {
  const query = normalizeCoffeeQuery(term);

  if (!query) return [];

  const matches = new Map<string, CoffeeSearchMatch & { score: number }>();
  const categoryByCoffeeId = new Map<string, CoffeeSearchMatch>();

  const addMatch = (category: CoffeeCategory, coffee: CoffeeType, score: number) => {
    const existing = matches.get(coffee.id);

    if (!existing || score < existing.score) {
      matches.set(coffee.id, { category, coffee, score });
    }
  };

  for (const category of coffeeCategories) {
    for (const coffee of category.items) {
      categoryByCoffeeId.set(coffee.id, { category, coffee });

      const fields = [coffee.name, coffee.en, coffee.id, ...(coffeeAliases[coffee.id] ?? [])].map(normalizeCoffeeQuery);
      const exact = fields.some((field) => field === query);
      const startsWith = fields.some((field) => field.startsWith(query));
      const includes = fields.some((field) => field.includes(query) || (field.length >= 2 && query.includes(field)));

      if (exact || startsWith || includes) {
        addMatch(category, coffee, exact ? 0 : startsWith ? 1 : 2);
      }
    }
  }

  const matchedFamilies = coffeeFamilies.map((family) => ({
    family,
    hit: family.keywords.map(normalizeCoffeeQuery).some((keyword) => query.includes(keyword)),
  }));
  const hasSpecificFamilyHit = matchedFamilies.some(({ family, hit }) => hit && !family.generic);

  for (const { family, hit } of matchedFamilies) {
    if (!hit || (family.generic && hasSpecificFamilyHit)) continue;

    family.ids.forEach((id, index) => {
      const item = categoryByCoffeeId.get(id);

      if (item) {
        addMatch(item.category, item.coffee, (family.scoreBase ?? 3) + index / 10);
      }
    });
  }

  return Array.from(matches.values())
    .sort((a, b) => a.score - b.score || a.coffee.name.length - b.coffee.name.length)
    .slice(0, limit)
    .map(({ category, coffee }) => ({ category, coffee }));
}

export function getExactCoffeeMatch(term: string): CoffeeSearchMatch | null {
  const query = normalizeCoffeeQuery(term);

  if (!query) return null;

  for (const category of coffeeCategories) {
    for (const coffee of category.items) {
      const fields = [coffee.name, coffee.en, coffee.id, ...(coffeeAliases[coffee.id] ?? [])].map(normalizeCoffeeQuery);

      if (fields.some((field) => field === query)) {
        return { category, coffee };
      }
    }
  }

  return null;
}

function normalizeCoffeeQuery(value: string) {
  return value
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "")
    .replace(/[·・_\-—]/g, "")
    .replace(/拿铁咖啡|美式咖啡|咖啡|饮品|饮料/g, (match) => {
      if (match === "拿铁咖啡") return "拿铁";
      if (match === "美式咖啡") return "美式";
      return "";
    });
}

export const caffeinePer100ml: Record<string, number> = {
  espresso: 100,
  americano: 45,
  latte: 30,
  cappuccino: 30,
  "flat-white": 40,
  cortado: 50,
  macchiato: 55,
  ristretto: 110,
  "long-black": 45,
  lungo: 70,
  piccolo: 42,
  breve: 32,
  "vanilla-latte": 28,
  "caramel-macchiato": 32,
  mocha: 25,
  "hazelnut-latte": 28,
  affogato: 20,
  "osmanthus-latte": 28,
  "coconut-latte": 25,
  "spanish-latte": 30,
  "sea-salt-latte": 28,
  "pistachio-latte": 26,
  "rose-latte": 28,
  "toffee-nut-latte": 28,
  yirgacheffe: 35,
  colombia: 38,
  kenya: 40,
  "panama-geisha": 30,
  mandheling: 42,
  brazil: 35,
  ethiopia: 36,
  guatemala: 38,
  "costa-rica": 36,
  honduras: 35,
  rwanda: 37,
  "iced-americano": 45,
  "cold-brew": 25,
  "nitro-coffee": 30,
  "iced-latte": 30,
  frappuccino: 15,
  "lemon-coffee": 35,
  "iced-oat-latte": 28,
  "iced-spanish-latte": 30,
  "cold-brew-latte": 24,
  "iced-mocha": 24,
  shakerato: 80,
  dirty: 45,
  "coconut-coffee": 20,
  "orange-americano": 35,
  "liquor-coffee": 20,
  "oat-latte": 28,
  "sparkling-americano": 35,
  "plum-americano": 30,
  "espresso-tonic": 42,
  "yuzu-americano": 35,
  "osmanthus-americano": 35,
  "pineapple-americano": 35,
  "mint-mocha": 24,
  "matcha-latte": 15,
  cocoa: 10,
  "black-tea-latte": 12,
  herbal: 0,
  "chai-latte": 10,
  "earl-grey": 8,
  "classic-milk-tea": 18,
  "bubble-milk-tea": 16,
  "jasmine-milk-tea": 14,
  "oolong-milk-tea": 18,
  "cheese-tea": 12,
  "brown-sugar-milk": 0,
  "fresh-lemon-tea": 8,
  "grape-lemon-tea": 6,
  "fresh-fruit-tea": 5,
  "passionfruit-tea": 6,
  "peach-oolong": 8,
  "cheese-grape": 6,
};

export const aiComments: Record<string, string> = {
  espresso: "一杯浓缩，两口闷完。像你处理紧急 bug 的速度。",
  americano: "大杯美式，血液变黑水。今天也是帮老板换保时捷努力的一天。",
  latte: "奶多咖少，职场温柔刀。老板看了都说好，同事看了想跳槽。",
  cappuccino: "奶泡蓬松如你的发际线，浓郁似你的加班怨气。",
  "flat-white": "澳白在手，格调我有。假装在墨尔本办公的打工魂。",
  cortado: "可塔朵，浓缩配一小口奶。像你开工资后短暂的快乐。",
  macchiato: "玛奇朵，甜头只有一点点，苦才是主旋律。",
  ristretto: "精萃，比普通浓缩还浓。像你在 deadline 前浓缩的焦虑。",
  "long-black": "长黑，水和浓缩各退一步，像你和需求方的暂时和解。",
  lungo: "加长浓缩，萃取时间变长，像会开着开着就多了三项待办。",
  piccolo: "短笛咖啡，小杯也有存在感，像一句轻飘飘但要命的改一下。",
  breve: "布雷卫，奶油感很足，现实感也很足。",
  "vanilla-latte": "香草拿铁，甜蜜如画的饼。喝完继续搬砖。",
  "caramel-macchiato": "焦糖玛奇朵，甜到忘记 KPI。短暂快乐像周五四点半。",
  mocha: "巧克力加咖啡，甜苦交织。像极了你周一早晨的心情。",
  "hazelnut-latte": "榛果拿铁，闻起来比你的年终总结还有层次。",
  affogato: "阿芙佳朵，冰淇淋浇浓缩。像用年终奖还花呗，甜蜜又心酸。",
  "osmanthus-latte": "桂花拿铁，花香盖过咖味。像你在工位上喷香水假装不在加班。",
  "coconut-latte": "生椰拿铁，打工人唯一的东南亚度假平替。",
  "spanish-latte": "西班牙拿铁，炼乳很甜，像你对今天还抱有一丝误会。",
  "sea-salt-latte": "海盐拿铁，咸甜交替，像工作里刚安慰完又被催。",
  "pistachio-latte": "开心果拿铁，听起来很开心，喝完还是要回消息。",
  "rose-latte": "玫瑰拿铁，香气很浪漫，工位很现实。",
  "toffee-nut-latte": "太妃榛果拿铁，甜得有层次，加班也有层次。",
  yirgacheffe: "耶加雪菲，花果香里找 bug。喝这杯的人觉得自己比产品经理懂生活。",
  colombia: "哥伦比亚单品，比你的职业规划还醇苦。",
  kenya: "肯尼亚咖啡，酸度拉满。像你看到同事升职时的心情。",
  "panama-geisha": "瑰夏一杯，钱包瘦十斤。喝的不是咖啡，是身份焦虑。",
  mandheling: "曼特宁，草本药草味。像一杯能治周一综合症的苦口良药。",
  brazil: "巴西单品，打工人的日常口粮，踏实得像你的工位。",
  ethiopia: "埃塞俄比亚，花果香很亮，像你下班前突然出现的自由幻觉。",
  guatemala: "危地马拉，坚果可可调，适合把复杂心情喝得更具体。",
  "costa-rica": "哥斯达黎加，干净又甜，像一份没有返工的需求。",
  honduras: "洪都拉斯，平衡得体，像你在会上努力维持的表情。",
  rwanda: "卢旺达，酸甜清楚，像今天每个锅的来源一样清楚。",
  "iced-americano": "冰美式，打工人的续命冰水。越冰越清醒，越清醒越想打开请假日历。",
  "cold-brew": "低温慢萃十二小时，就像你等年终奖的耐心。",
  "nitro-coffee": "氮气咖啡，绵密如你的画饼能力。看起来高级，喝起来上头。",
  "iced-latte": "冰拿铁，夏天的职场安慰剂。暂时忘记 deadline。",
  frappuccino: "星冰乐，冰沙里掺了点咖啡。像你在工作中掺了点摸鱼。",
  "lemon-coffee": "冻柠咖啡，酸甜苦的混合体。像你对公司的感情。",
  "iced-oat-latte": "冰燕麦拿铁，健康人设在线，待办列表也在线。",
  "iced-spanish-latte": "冰西班牙拿铁，甜得很会营业，像你回收到时的语气。",
  "cold-brew-latte": "冷萃拿铁，温柔里带后劲，像下午三点后的需求提醒。",
  "iced-mocha": "冰摩卡，巧克力负责甜，咖啡负责让你面对现实。",
  shakerato: "冰摇浓缩，摇出来的是泡沫，也是你今天最后一点耐心。",
  dirty: "脏脏咖啡，像你桌面上三个月没整理的文件。但味道确实上头。",
  "coconut-coffee": "椰子水咖啡，清爽得像离职申请通过的那一秒。",
  "orange-americano": "橙子美式，酸得很阳光。像你把想下班包装成积极营业。",
  "liquor-coffee": "酒香咖啡，成年人的续命要加点料。",
  "oat-latte": "燕麦拿铁，健康人设立起来了，夜宵照吃不误。",
  "sparkling-americano": "气泡美式，连咖啡都开始内卷口感了。",
  "plum-americano": "话梅美式，酸甜咸苦都有，像你一天的工作情绪。",
  "espresso-tonic": "汤力咖啡，气泡很清醒，像突然被拉进会的那一秒。",
  "yuzu-americano": "柚子美式，清爽是真的，想下班也是真的。",
  "osmanthus-americano": "桂花美式，香得很克制，像你没有发出去的吐槽。",
  "pineapple-americano": "菠萝美式，热带感很足，工位感也没少。",
  "mint-mocha": "薄荷摩卡，凉意和甜意都有，现实负责补上苦味。",
  "matcha-latte": "抹茶拿铁，不是咖啡但很懂装忙。",
  cocoa: "可可，成年人偶尔也需要假装自己还有童年。",
  "black-tea-latte": "红茶拿铁，咖啡因换个马甲继续支配你。",
  herbal: "草本饮品，零咖啡因，但焦虑不减半。",
  "chai-latte": "印度奶茶，香料够多，足以盖住今天的班味。",
  "earl-grey": "伯爵茶，优雅地清醒，体面地加班。",
  "classic-milk-tea": "经典奶茶，甜度给了安慰，工作量没有。",
  "bubble-milk-tea": "珍珠奶茶，嚼的是珍珠，咽下去的是今天的耐心。",
  "jasmine-milk-tea": "茉莉奶绿，香气很清新，打工状态很陈旧。",
  "oolong-milk-tea": "乌龙奶茶，茶感在线，脑子也勉强在线。",
  "cheese-tea": "芝士茶，奶盖很厚，像你给情绪盖上的那层体面。",
  "brown-sugar-milk": "黑糖鲜奶，甜得很认真，现实依然不放糖。",
  "fresh-lemon-tea": "鲜切柠檬茶，酸得清醒，像刚看完新的需求变更。",
  "grape-lemon-tea": "葡萄鲜切柠檬茶，果香很足，足以短暂盖住班味。",
  "fresh-fruit-tea": "满杯水果茶，颜色很努力，像你今天的积极营业。",
  "passionfruit-tea": "百香果茶，酸甜一起上班，情绪也一起打卡。",
  "peach-oolong": "白桃乌龙，闻起来像周末，喝完还是周中。",
  "cheese-grape": "芝士葡萄，快乐很浓，工位距离自由依然很远。",
};

const baseToxicQuotes = [
  "这杯咖啡的苦，远不及你周一早会时的职业微笑。",
  "你以为喝的是咖啡？不，你喝的是加班的燃料。",
  "打工人的血液里，70% 是咖啡，30% 是已读不回的自控力。",
  "咖啡续命，deadline 催更，今天的你依然很会在线营业。",
  "别问为什么这么苦，看看你的工资条。",
  "今日咖啡已到账，精神状态依然待审批。",
  "你不是离不开咖啡，你是离不开假装还能干。",
  "这一口下去，KPI 没完成，但心率先完成了。",
  "你喝下去的是拿铁，咽下去的是需求变更。",
  "加班不会让你变富，但咖啡会让你误以为还能撑。",
  "今天的精神状态：杯子是满的，人是空的。",
  "咖啡因开始工作了，你的工资还在试用期。",
  "这杯下肚，脑子上线，表情继续归公司统一管理。",
  "老板画饼，你喝咖啡，碳水和咖啡因共同维持幻觉。",
  "如果努力有用，咖啡店早就上市感谢你了。",
  "你不是困，你只是被早会加载得有点慢。",
  "这杯咖啡叫醒了你，也叫不醒装睡的预算。",
  "喝之前是社畜，喝之后是高清社畜。",
  "咖啡很苦，但比不上需求会后补充的那一句。",
  "今天也是靠咖啡把想下班包装成专业的一天。",
  "你以为你在续命，其实你在延长在线时间。",
  "杯盖一合，情绪一收，继续做个成熟的打工人。",
  "咖啡香气四溢，离职念头也很清晰。",
  "这杯喝完，问题不会消失，只会变得更醒目。",
  "别人喝咖啡提神，你喝咖啡是为了继续装作听懂。",
  "焦虑兑上冰块，就是今日特调。",
  "你和这杯咖啡的共同点：都被榨过。",
  "咖啡是苦的，绩效沟通是回甘都没有的。",
  "这一口叫醒大脑，下一秒继续被会议按回去。",
  "不要问咖啡多少钱，问就是比你的时薪有尊严。",
  "你今天喝的是美式，过的是没事在线式。",
  "杯子里的泡沫，比晋升承诺还真实一点。",
  "喝完这一杯，继续用积极语气说出我再看看。",
  "咖啡救不了全局，只能先救下午三点这个局部变量。",
  "今日份续命完成，今日份意义仍在加载。",
  "一杯咖啡下去，心跳有了，方案还在空白页装死。",
  "别担心睡不着，反正明天醒来也要上班。",
  "咖啡越浓，越像你对甲方的怨气。",
  "你喝的是冷萃，心是热的，工资是凉的。",
  "这杯的酸度像团队协作，入口就让人皱眉。",
  "奶泡很厚，像你假装听懂战略会的表情。",
  "每一口都在提醒你：成年人的快乐按毫升计费。",
  "你不是没电了，你是长期欠费运行。",
  "这杯咖啡没有解决问题，但让你清醒地面对问题。",
  "早八的你和冰美式，都是被迫上岗。",
  "咖啡豆被研磨，你被周报研磨，大家都有光明未来。",
  "喝下它，继续成为组织架构里一颗安静的小齿轮。",
  "今日咖啡风味：前调赶地铁，中调赶进度，尾调赶紧下班。",
  "咖啡因让你醒着，工资让你冷静。",
  "你不是需要休息，你只是需要一个不用解释的周末。",
  "这杯下去，脑子开机，心情蓝屏。",
  "苦味很高级，像你朋友圈里不方便明说的班味。",
  "咖啡杯见证了你比 OKR 更稳定的在线状态。",
  "今天的你被咖啡扶起来，又被需求按回工位。",
  "喝咖啡不是习惯，是求生本能穿了件体面的外套。",
  "你以为这是奖励自己，其实是给公司设备续航。",
  "这杯咖啡越喝越清醒，越清醒越想请假。",
  "香气扑鼻，余额冷静，付款成功后人也冷静了。",
  "你和咖啡机都很忙，区别是它还有定期清洁。",
  "咖啡的回甘短暂，工作的回旋镖持久。",
  "这一杯敬明天，虽然明天也不一定放过你。",
  "你喝下的每一口，都在替工位上的待机模式续电。",
  "冰块融化了，KPI 没有。",
  "咖啡杯边的水珠，是打工人替自己偷偷开的静音模式。",
  "今天也是用咖啡因和礼貌撑起职场体面的一天。",
  "如果入职申请有撤回键，你大概会先点三次确认。",
  "杯底见了，需求还没见底。",
  "你不是迷恋咖啡，你只是迷恋三分钟没人找你的错觉。",
  "这杯咖啡很懂事，至少它真的有在发挥作用。",
  "喝完这杯，继续假装自己的职业规划不是随机游走。",
  "咖啡在胃里，弹幕在心里，笑容在会议里。",
  "今天的生产力来自咖啡，归属感来自幻觉。",
  "你说它苦，它说先看看你的需求池再评价。",
  "这杯的层次很丰富：先提神，再心慌，最后继续干活。",
  "咖啡店的座位很软，现实的工位很硬。",
  "喝下去的是咖啡，吐出来的是收到收到。",
  "你以为你在品风味，其实你在评估还能撑几小时。",
  "咖啡杯很小，装不下你对工作的全部意见。",
  "今日续命口味：少糖，多点表情管理。",
  "这杯咖啡至少诚实地苦，不像有些会议包装得很甜。",
  "喝完以后你会更清醒地知道：问题不是困。",
  "咖啡帮你熬过白天，手机帮你熬过夜晚。",
  "你把咖啡喝完了，咖啡也把你看透了。",
  "醒来、打卡、喝咖啡、假装一切都可控。",
  "这杯是你的临时补丁，工位系统仍有大量 warning。",
  "咖啡因在奔跑，工资条在原地踏步。",
  "杯里的拉花很漂亮，像你精心维护的职场人设。",
  "这一口下去，至少你的已读不回更有底气了。",
  "喝咖啡前想辞职，喝咖啡后想有计划地辞职。",
  "你今天不是需要灵感，你只是需要别再开会。",
  "咖啡是打工人的回血包，只是副本难度没有下降。",
  "每次扫码录入，都是对今天还没倒下的纪念。",
  "这杯饮品认证成功：它能喝，你也还能演。",
  "这杯咖啡像你的职业发展，闻起来有前景，喝下去全是现实。",
  "今天的咖啡因余额充足，银行卡余额保持低调。",
  "你以为你在摸鱼，其实你只是在给大脑清缓存。",
  "咖啡让你醒来，工作让你想重新睡回去。",
  "这一杯的苦味很稳，像公司永远不会消失的流程。",
  "喝完这杯，你会更清楚地知道今天哪里不太对劲。",
  "成年人的早晨，靠咖啡开机，靠工资按住关机键。",
  "这杯咖啡是热的，职场关系是冷的。",
  "你今天不是状态不好，你只是对现实过敏。",
  "咖啡杯很轻，装着的班味很重。",
  "今天的你没有迟到，只是大脑还在通勤路上。",
  "这杯下去，眼睛亮了，屏幕也更刺眼了。",
  "喝咖啡是为了清醒，清醒是为了继续被安排。",
  "你不是缺动力，你缺的是不用上班的理由。",
  "咖啡因让你暂时上线，绩效让你学会少说两句。",
  "这杯咖啡的香气，比领导的承诺更接近真实。",
  "喝完以后请继续保持专业，哪怕内心已经退出会议。",
  "你的今日目标：活着下班，并显得很有执行力。",
  "这杯咖啡比你的工作群更懂得适可而止。",
  "越喝越清醒，越清醒越发现今天又是循环任务。",
  "咖啡没有骗你，它从一开始就承认自己很苦。",
  "你和这杯咖啡都在降温，只是它比你更快被喝完。",
  "今天的心情像无糖美式，入口就知道没有惊喜。",
  "这杯续的是命，耗的是对生活的想象力。",
  "咖啡可以加奶，工作不能加倍工资。",
  "你不是在奋斗，你是在用咖啡因抵押明天的精神状态。",
  "杯口的热气升起来了，你的晋升空间还在原地。",
  "这一口下去，脑子开机，工位锁屏解除。",
  "你以为咖啡救了你，其实只是让你更清醒地加班。",
  "这杯咖啡没有废话，但至少没有要求你写周报。",
  "早会前喝咖啡，是成年人对自我的最后一点保护。",
  "今天的你像冰美式，看起来冷静，实际很苦。",
  "咖啡机都知道定时休息，你还在假装自己是永动机。",
  "这杯咖啡提醒你，真正上头的不是咖啡因，是工作量。",
  "需求变更像咖啡渣，总会在最后一口出现。",
  "你不是想喝咖啡，你是想拥有三分钟不被打扰。",
  "喝下这杯，继续把离谱需求翻译成好的收到。",
  "咖啡越贵，越显得你对今天还有期待。",
  "这杯是职场防御药水，效果持续到下一个会议通知。",
  "你今天的精神状态像外卖咖啡，路上已经洒了一半。",
  "咖啡不是解药，是把问题调成高清模式。",
  "喝完以后你会发现，困只是问题里最温柔的那一个。",
  "这杯咖啡的后劲，比请假理由还难编。",
  "职场最稳定的三件事：会议、加班、你手里的咖啡。",
  "你把咖啡当救命绳，公司把你当待办项。",
  "这杯下去，打工人的操作系统完成热更新。",
  "今日风味：前调赶地铁，中调赶进度，尾调赶紧下班。",
  "咖啡很浓，像你不敢发出来的那段话。",
  "你不是效率低，你只是被太多无效沟通稀释了。",
  "喝咖啡前是低电量，喝咖啡后是高功耗。",
  "这杯咖啡能提神，但不能提高预算。",
  "今天的工位很安静，安静得像你的加薪通知。",
  "咖啡杯里的冰块在融化，你的耐心也是。",
  "喝完这杯，继续把今天过成项目排期。",
  "你不是没有想法，只是想法被会议纪要改成了待跟进。",
  "这杯咖啡的苦，像你打开工资单时的加载动画。",
  "咖啡可以续杯，假期不能。",
  "你今天的笑容很职业，像自动回复一样稳定。",
  "这杯饮品没有给你答案，但给了你继续点头的表情管理。",
  "醒着不代表清醒，清醒不代表想上班。",
  "咖啡把你从床上捞起来，工作把你按回现实里。",
  "这一杯是给身体的，下一场会是给耳机降噪的表现机会。",
  "你的大脑已经启动，情绪仍在维护中。",
  "这杯咖啡的层次，比公司审批流程少一点。",
  "别嫌它苦，至少它没有要求你对齐一下。",
  "咖啡因负责提神，成年人负责假装没事。",
  "喝完这杯，你会拥有短暂的生产力和长久的心慌。",
  "今天的你很像咖啡杯，外表完整，内部空了又满。",
  "这杯咖啡没有绩效目标，所以它比你自由。",
  "如果努力能换钱，你的咖啡杯早该退休了。",
  "喝咖啡是为了面对工作，工作是为了买下一杯咖啡。",
  "这杯的香气像新项目启动会，听起来很美，后面很累。",
  "你不是在内耗，你是在做无薪的心理加班。",
  "咖啡救不了拖延，但能让你更精神地刷新待办。",
  "这杯咖啡提醒你，周一不是一天，是一种状态。",
  "你的灵感没有枯竭，它只是拒绝为这个需求服务。",
  "咖啡入口的一秒，是今天最像生活的时刻。",
  "这杯下去，继续把想下班折叠进待办清单。",
  "你以为今天会不一样，咖啡杯笑而不语。",
  "咖啡很懂你，所以它选择不多问。",
  "这杯饮品可以加糖，职场现实不提供这个选项。",
  "喝完以后你会更像一个成年人：清醒，克制，会说收到。",
  "你的工作状态像冷掉的拿铁，还能喝，但不太体面。",
  "咖啡续命成功，任务列表拒绝配合。",
  "今天的你离财富自由，只差财富和自由。",
  "这杯咖啡的价格，提醒你上班也是一种消费。",
  "喝下去的是咖啡因，冒出来的是强装镇定。",
  "你不是不想努力，你是不想再努力给别人看。",
  "咖啡的苦是风味，工作的苦是默认配置。",
  "这杯让你醒了，但没有让你想通。",
  "你的日程像浓缩咖啡，量不大，但压强很高。",
  "喝咖啡前想躺平，喝咖啡后想坐着躺平。",
  "这杯咖啡是今天的存档点，虽然没有读档功能。",
  "别问今天过得怎么样，问就是还能扫码录入。",
  "咖啡没有解决方案，但它至少没有提出新问题。",
  "你今天的积极，主要来自咖啡和职业素养。",
  "这杯咖啡叫做暂时不辞职。",
  "喝完它，继续在群里做情绪稳定的成年人。",
  "你的心情像打包咖啡，表面封得很好，里面晃得厉害。",
  "咖啡让你醒着，生活让你想静音。",
  "这杯下肚，继续把不合理解释成挑战。",
  "你不是需要鼓励，你需要一个没有消息提醒的下午。",
  "咖啡杯见底了，待办事项还在繁殖。",
  "这杯咖啡的有效期，比领导的耐心还短。",
  "你今天的努力很明显，回报假装没看见。",
  "喝咖啡像打补丁，系统能跑，但问题还在。",
  "这杯是你的临时士气，过期时间是下个会前。",
  "职场没有奇迹，只有咖啡因和截止日期。",
  "你不是困在工作里，你是被工资条温柔挽留。",
  "这杯咖啡比大多数需求更有边界感。",
  "喝完以后，继续用一脸平静处理人类迷惑行为。",
  "你今天的脑子不是慢，是被同步会议占用了带宽。",
  "咖啡很苦，但至少没有让你写复盘。",
  "这杯饮品让你醒来，也让你看清没醒的人不用回消息。",
  "你和咖啡的关系很纯粹：它给清醒，你给钱。",
  "今日份职场体面，由咖啡因赞助播出。",
  "这杯咖啡的余味，像未读消息一样挥之不去。",
  "喝完以后，继续做一个看似稳定的高压容器。",
  "你不是在等下班，你是在等今天停止消耗你。",
  "咖啡的拉花会散，职场的锅会传。",
  "这杯咖啡让你短暂相信，自己还能再抢救一下。",
  "你今天的元气是借来的，明天记得还。",
  "喝咖啡前人很困，喝咖啡后心很慌，工作很满意。",
  "这杯的酸味像反馈意见，越品越不舒服。",
  "咖啡香气很友好，前提是不打开工作群。",
  "你不是不爱工作，你只是更爱不用工作的自己。",
  "这杯咖啡像周报，存在感很强，实际价值看心情。",
  "喝完这杯，继续把低电量伪装成专注。",
  "你今天的自控力，主要体现在没有当场关机。",
  "咖啡没有请你振作，它只是默默增加了心跳。",
  "这杯饮品的甜度可选，今天的难度默认专家模式。",
  "你不是状态差，只是长期运行在省电模式。",
  "喝咖啡是短暂回血，上班是持续扣血。",
  "这杯咖啡至少陪你加班，不像有些承诺只陪你画饼。",
  "今天的你表面在工作，内心在刷新退出按钮。",
  "咖啡因给了你速度，现实给了你阻力。",
  "这杯下去，继续做组织里那个会自动回复的人。",
  "你不是没热情，你的热情被审批流程冷却了。",
  "咖啡杯很小，放不下你对这个项目的全部质疑。",
  "这杯咖啡像绩效面谈，入口紧张，回味复杂。",
  "喝完以后，你会拥有更快的打字速度和更慢的下班时间。",
  "今天也是把事情很多说成挺充实的一天。",
  "咖啡让人清醒，但清醒的人通常更想请假。",
  "这杯的回甘很短，像周末。",
  "你的职业规划像杯底咖啡渍，看得出来曾经很浓。",
  "喝咖啡不是仪式感，是成年人给自己的续航通知。",
  "这杯咖啡证明：你还没有放弃，只是放低了期待。",
  "醒来以后最残酷的事，是发现工作也醒了。",
  "你今天的安静不是认可，是咖啡还没生效。",
  "咖啡在杯里旋转，你在项目里打转。",
  "这杯饮品让你继续在线，至于脑子是否在线另说。",
  "喝完这杯，继续把离谱听成机会。",
  "你不是没有情绪，你只是学会了把情绪静音。",
  "咖啡是苦的，没关系，反正你很熟。",
  "这杯咖啡像老板的消息，出现得及时，压力也及时。",
  "今天的你很努力，钱包表示没有收到通知。",
  "喝咖啡是为了续命，不是为了和工位培养感情。",
  "这杯下去，至少你能更有精神地等待下班。",
  "你的耐心像冰块，正在杯子里逐渐消失。",
  "咖啡不会背叛你，它只会让你晚上睡不着。",
  "这杯咖啡把你从梦里拉出来，交给现实继续处理。",
  "你不是需要正能量，你需要少一点工作量。",
  "喝下这杯，继续把荒谬包装成协作。",
  "今天的职场关键词：表面积极，内部告急。",
  "咖啡能醒脑，不能醒悟预算不够还要做完的事实。",
  "这杯饮品很职业，苦得很有分寸。",
  "你今天的大脑状态：已读，不回。",
  "喝完以后，你会更像自己，也更像打工人。",
  "这杯咖啡告诉你，清醒本身就是一种代价。",
  "每一口都在提醒你：你不是缺咖啡，是缺假期。",
  "咖啡喝完了，成年人模式自动续订。",
  "今天的你没有崩，是因为咖啡杯还没空。",
  "这杯饮品没有 KPI，但完成度比你高。",
  "喝下去的是提神，留下来的是想静静。",
  "你不是在上班，你是在把一天切成一个个会议块。",
  "咖啡负责叫醒你，现实负责让你后悔醒来。",
  "这杯下肚，继续假装消息提示音不是新活敲门。",
  "你的职业素养，是把想下班说成我再看看。",
  "咖啡的香味很短暂，工作群的存在很漫长。",
  "这杯咖啡像同事的建议：听着有道理，落地靠你自己。",
  "喝完以后，继续在忙碌里寻找一点自我安慰。",
  "你今天没有摆烂，只是执行力暂时离线。",
  "咖啡能让你快一点，但不能让世界讲道理。",
  "这杯是成年人的魔法药水，副作用是更清楚地知道自己累。",
  "你不是在收集咖啡，你是在收集还没倒下的证据。",
  "这杯咖啡像未读消息，越看越清醒，越清醒越不想点开。",
  "今天的你不是效率低，是 CPU 被无效沟通占满了。",
  "咖啡可以加奶，工作只能加码。",
  "你以为自己在赶进度，其实是在追赶一个会移动的终点。",
  "这杯饮品没有解决问题，但至少让你更有精神地面对问题。",
  "上班最稳定的收益，是稳定地想下班。",
  "咖啡续的是命，会议耗的是魂。",
  "你不是没有目标，你只是被太多临时需求改了路线。",
  "这杯的苦味很真实，像老板说的简单改一下。",
  "喝完这杯，继续把心累翻译成好的收到。",
  "成年人的清醒，是知道咖啡只能救今天，救不了排期表。",
  "你的工位像充电桩，插着你，亮着灯，但不一定有电。",
  "这杯咖啡不贵，贵的是你为它配套付出的加班时间。",
  "今日状态：脑子上线，表情请假。",
  "咖啡因负责提神，KPI 负责提命。",
  "你不是不想努力，是努力的方向总被重新对齐。",
  "这杯饮品很像需求文档，颜色很漂亮，细节很危险。",
  "喝咖啡前你困，喝咖啡后你困得更清醒。",
  "职场最浪漫的事，是准点下班还没人找你。",
  "这杯咖啡像组织架构，层次很多，重点很苦。",
  "你今天的笑容很专业，专业到本人都不太认识。",
  "喝完这口，继续把荒唐当流程处理。",
  "咖啡告诉你还能撑，工资告诉你先别辞。",
  "你不是情绪稳定，你只是消息回得比较慢。",
  "这杯饮品的香气有限，工作群的压迫感无限。",
  "你在表格里寻找秩序，生活在旁边偷偷乱套。",
  "咖啡是液体闹钟，响完以后还是要上班。",
  "今天的你不是没状态，是状态被开会稀释了。",
  "这杯像年终总结，前半段浓烈，后半段靠想象。",
  "喝下去的是美式，咽下去的是现实。",
  "你不是喜欢忙，你只是暂时没有别的选项。",
  "工作最会画饼，咖啡最会让你继续吃。",
  "这杯饮品很懂事，苦得不像某些需求那么突然。",
  "你今天还很稳定，主要是因为还没到下班前最后一小时。",
  "咖啡能帮你睁眼，不能帮你看开。",
  "你不是在摸鱼，你是在给大脑做灾后重建。",
  "这杯咖啡像项目排期，看起来满，喝起来慌。",
  "成年人的日常，是把想逃跑说成出去透口气。",
  "你以为自己缺灵感，其实是缺一个不改需求的人。",
  "咖啡的苦是诚实的，职场的甜多半是话术。",
  "这杯下去，继续在群消息里寻找撤回按钮。",
  "今天的你像自动保存，没什么进展，但一直在运行。",
  "你不是能力不行，是工作量的想象力太行。",
  "咖啡香得很短，ddl 长得很稳定。",
  "这杯饮品没有口号，只有续航。",
  "你今天的努力很珍贵，只是系统暂未识别。",
  "喝完以后，继续把想吐槽压缩成一个句号。",
  "职场教会你最重要的技能，是把不理解说成我理解了。",
  "咖啡杯见过你最真实的表情，摄像头只见过你的职业微笑。",
  "这杯像日报，写的时候清醒，看的人随缘。",
  "你不是在加班，你是在给明天的自己制造悬念。",
  "喝咖啡是主动续命，加班是被动扣费。",
  "今天也是把所有事情折叠进浏览器标签页的一天。",
  "你不是没脾气，你只是学会了把脾气存草稿。",
  "这杯饮品不会升职，但会陪你熬到下一个版本。",
  "咖啡越浓，你对需求的理解越清晰，也越想静音。",
  "你今天的工位很安静，除了心里一直在离职。",
  "这杯下去，你还是你，只是心跳更努力。",
  "工作像冷掉的咖啡，已经不香了，但还得喝完。",
  "你不是追求完美，你是在追赶别人临时改出来的完美。",
  "咖啡能让你醒，不能让项目自己醒悟。",
  "这杯饮品的价值，在于它没有问你为什么还没做完。",
  "今日份清醒已到账，快乐仍在审批中。",
  "你不是没有斗志，是斗志被通知栏耗尽了。",
  "咖啡负责把你拉回现实，现实负责再次把你按住。",
  "这杯像老板的鼓励，听着暖，后面一般有活。",
  "你今天的耐心，是用咖啡和少说两句共同充值的。",
  "喝完这杯，继续在不合理里保持礼貌。",
  "职场最奢侈的自由，是能把消息晚点回。",
  "咖啡的存在证明，人类并不适合自然醒后直接工作。",
  "这杯饮品没有情绪价值，只有功能性续命。",
  "你不是想偷懒，你只是想短暂拥有自己。",
  "今天的你像加载页，转了很久，但没人知道在加载什么。",
  "咖啡不是解药，是暂时不倒的证据。",
  "这杯像绩效目标，越喝越觉得自己被定义了。",
  "你没有被工作打败，你只是被它每天分期付款。",
  "喝完以后，你会更清醒地意识到还要继续。",
  "职场的温柔，是别人说辛苦了之后不再追加需求。",
  "这杯咖啡有入口，工作没有出口。",
  "你不是在内耗，是在和不确定性长期合租。",
  "咖啡让你看起来在线，心里那部分另算。",
  "这杯饮品的后劲，是凌晨还在回想白天的破事。",
  "今天的你不是低电量，是电池健康度感人。",
  "你以为咖啡在救你，其实它只是给工作续费。",
  "这杯像周一，开头就知道不容易。",
  "成年人的礼貌，是杯子空了还能继续点头。",
  "你不是抗压能力强，你只是暂时没有空碎。",
  "咖啡再香，也盖不住需求变更的味道。",
  "这杯饮品适合上班喝，因为它和你一样被迫营业。",
  "你今天的灵感，可能被排期吓回去了。",
  "喝下这杯，继续做一个温和的任务处理器。",
  "职场最真实的清醒，是知道很多事急也没用，但还是得急。",
  "咖啡是给身体的补丁，假期才是系统升级。",
  "这杯像沟通记录，表面顺滑，底部全是沉淀。",
  "你不是不积极，你只是对无意义的热闹过敏。",
  "今天的你很稳定，稳定地想消失五分钟。",
  "咖啡没有改变排期，只改变了你打字的速度。",
  "这杯饮品很体面，至少苦得没有甩锅。",
  "你不是累，你是被太多小事同时登录了。",
  "喝完这杯，继续把不想做翻译成我来跟进。",
  "职场人的自律，是困到模糊还知道先保存文件。",
  "这杯咖啡像周会，前味平平，后劲绵长。",
  "你今天的微笑，是工牌以外最重要的装备。",
  "咖啡只能帮你撑住，不能帮你撑起整个部门。",
  "这杯下去，清醒是清醒了，问题也更清楚了。",
  "你不是在等待灵感，是在等待一个不用返工的世界。",
  "工作把你磨圆了，咖啡把你暂时磨亮了。",
  "这杯饮品没有升职空间，但至少有杯盖。",
  "你今天的情绪，已被压缩上传到草稿箱。",
  "咖啡让人专注，专注地发现事情更多了。",
  "这杯像预算，刚开始很满，用着用着就见底。",
  "你不是缺方法，是缺一个不半夜发消息的环境。",
  "喝完以后，继续在会议纪要里寻找人类文明。",
  "职场最残酷的循环，是做完一个就自动刷新三个。",
  "这杯咖啡不懂业务，所以它比你轻松。",
  "你今天的战斗力，来自咖啡因和被迫负责。",
  "咖啡很苦，但它不会要求你同步一下进展。",
  "这杯饮品适合配需求变更，苦味更有层次。",
  "你不是没想法，只是说出来又会变成你的活。",
  "喝咖啡是为了醒着营业，不是醒着团建。",
  "今天的你像共享文档，大家都能改，没人负责最终版。",
  "这杯下去，继续把被安排说成可以。",
  "咖啡不会劝你努力，它知道你已经够累了。",
  "你不是讨厌上班，你只是讨厌上班里那些额外的上班。",
  "这杯饮品像月报，存在的意义主要是证明你存在过。",
  "你今天没有直接下线，是因为会议室信号还行。",
  "咖啡给你一点冲动，工资条负责踩刹车。",
  "这杯像老板的临时想法，出现突然，影响深远。",
  "成年人的体面，是在想骂人的时候改成收到。",
  "你不是工作狂，你只是被工作狂热地追着跑。",
  "喝完这杯，继续把混乱整理成看起来有序的表格。",
  "职场最稀缺的资源，不是预算，是安静十分钟。",
  "这杯咖啡像你的人设，表面稳定，内部高压。",
  "你今天的快乐，被登录验证码拦截了。",
  "咖啡的使命很简单，让你醒着面对复杂。",
  "这杯饮品没有指责你，它只是默默见证你又晚睡了。",
  "你不是在追梦，是在坚持不要当场退出群聊。",
  "喝咖啡前想睡觉，喝咖啡后想下班。",
  "今天的你很专业，专业到把失望都说得很客气。",
  "这杯像老板画的饼，闻起来香，吃起来空气感很强。",
  "你不是没成长，你只是长出了更多快捷回复。",
  "咖啡杯里有液体，工作群里有压力。",
  "这杯下去，继续把未知风险说成可控。",
  "成年人的节制，是只买一杯咖啡，不买辞职的冲动。",
  "你今天的状态，不适合被问进展。",
  "咖啡给身体装了加速器，生活给前方设置了红灯。",
  "这杯饮品不懂 KPI，所以喝起来比较纯粹。",
  "你不是缺动力，是动力被流程卡住了。",
  "喝完这杯，继续把真实想法藏在会议静音里。",
  "职场最常见的奇迹，是预算没有，要求很高。",
  "这杯咖啡像转正答辩，喝完也不确定自己稳不稳。",
  "你今天很努力，只是努力对象可能不太值得。",
  "咖啡不能改变世界，但能让你更清楚地看到世界没变。",
  "这杯饮品像工作交接，入口顺滑，后面全靠猜。",
  "你不是在拖延，是在给焦虑发酵时间。",
  "喝下这口，继续把普通一天过成轻度闯关。",
  "职场人的浪漫，是没人打扰地吃完一顿午饭。",
  "这杯咖啡很真诚，不像某些排期那样假装充裕。",
  "你今天的情绪稳定，主要因为还没打开那个文档。",
  "咖啡越冰，现实越烫。",
  "这杯像紧急需求，喝得越快，心越慌。",
  "你不是玻璃心，是每天都被小石子砸。",
  "喝完以后，继续在体面和发疯之间找平衡。",
  "职场的成长，很多时候是从敢怒不敢言到懒得怒。",
  "这杯饮品提醒你，醒着不等于掌控。",
  "你今天的脑子很忙，忙着假装没听见离谱要求。",
  "咖啡是成年人允许自己短暂依赖的东西。",
  "这杯下去，继续把明明不行说成我试试。",
  "你不是没有边界感，你的边界被待办事项踩平了。",
  "工作像浓缩咖啡，量不大，但足够让人心跳加速。",
  "这杯饮品的安静，胜过很多无效沟通。",
  "你今天的耐心值，正在以肉眼不可见的速度下降。",
  "咖啡可以续杯，耐心不一定可以。",
  "这杯像团建通知，表面轻松，内心复杂。",
  "你不是不会社交，是不想把下班时间也交出去。",
  "喝完这杯，继续把生活按优先级排到最后。",
  "职场最可怕的词，是顺手。",
  "这杯咖啡知道你累，但它也只能尽力。",
  "你今天的理智，是靠咖啡杯壁撑住的。",
  "咖啡不问晋升路线，咖啡只问你还醒不醒。",
  "这杯饮品像项目复盘，越品越觉得问题不在咖啡。",
  "你不是焦虑，是对未来有过多实时预览。",
  "喝下去的是冰拿铁，升上来的是未完成事项。",
  "今天的你像线上系统，表面运行，后台报警。",
  "这杯像月初计划，看起来很美，执行起来很现实。",
  "你不是想逃避工作，是想逃避工作带来的全部附加剧情。",
  "咖啡可以提神，不能提高别人理解需求的能力。",
  "这杯饮品很克制，苦得比职场话术直接。",
  "你今天的礼貌，是经过职业训练的文明成果。",
  "喝完以后，继续做一个会自动归档情绪的人。",
  "职场最贵的成本，是反复解释同一件事。",
  "这杯咖啡像改稿意见，越到后面越上头。",
  "你不是没有兴趣，只是兴趣没有报销额度。",
  "咖啡让眼睛睁开，现实让心门关上。",
  "这杯下肚，继续把不合理需求拆成合理步骤。",
  "你今天不是没有灵感，是灵感被权限限制了。",
  "喝咖啡是日常补给，辞职念头是隐藏任务。",
  "这杯饮品像公司制度，写得清楚，执行看命。",
  "你不是在假装积极，你是在保护自己的工资卡。",
  "咖啡的香气消散后，工位还是那个工位。",
  "这杯像凌晨的脑子，明明很累，还在高速运转。",
  "你今天的目标很朴素，别出大事就行。",
  "喝完这杯，继续把复杂关系处理成简单表情。",
  "职场人的小确幸，是没有新增红点的一分钟。",
  "这杯咖啡没有冒犯你，它只是和你的生活同频了。",
  "你不是抗拒沟通，是抗拒无效沟通的重复播放。",
  "咖啡越甜，越显得现实没放糖。",
  "这杯饮品像版本更新，修复有限，问题新增。",
  "你今天的职业素养，是没把真实想法发进群里。",
  "喝下这口，继续在职责边界上走钢丝。",
  "职场最难喝的不是黑咖啡，是别人递过来的锅。",
  "这杯咖啡适合配周报，苦中带一点形式主义。",
  "你不是没有执行力，是执行对象一直变。",
  "咖啡不会安慰你，但也不会催你交付。",
  "这杯像下午三点的电脑，开着，但不太想工作。",
  "你今天的职业规划，是先撑到饭点。",
  "喝完以后，继续把快疯了说成状态还行。",
  "职场最有效的保健，是按时下班，可惜经常缺货。",
  "这杯饮品像领导的赞许，出现之后通常还有下一句。",
  "你不是不懂人情世故，是懂了以后更累。",
  "咖啡因是临时工，待办事项才是正式员工。",
  "这杯下去，继续把紧急但不重要的事做得像老板亲临。",
  "今天的你像办公椅，承受很多，轮子还得照常转。",
  "你不是在消极，是在精准评估现实的荒唐程度。",
  "咖啡是短效药，周末是处方药。",
  "这杯咖啡没有 KPI，所以它比你自由。",
  "你今天的心态，已从积极处理切换到礼貌存活。",
  "喝完这杯，继续在日程表里寻找呼吸感。",
  "职场最动听的话，是今天不用开会。",
  "这杯饮品像审批流程，走完以后热情也凉了。",
  "你不是拖后腿，是被项目拖着走。",
  "咖啡给你续航，老板给你续需求。",
  "这杯像离职信草稿，存在心里，暂不提交。",
  "你今天没有发疯，是因为浏览器还有未保存内容。",
  "喝下去的是咖啡，压下去的是那句凭什么。",
  "职场最常见的安慰，是大家都一样惨。",
  "这杯咖啡很公平，谁喝都苦。",
  "你不是没有创造力，是创造力不想服务无效需求。",
  "咖啡杯空了，待办事项还在满格运行。",
  "这杯饮品像例会，喝之前以为很短，结束发现时间没了。",
  "你今天的平静，是暴风雨前的自动静音。",
  "喝完以后，继续在合理化里寻找心理支撑。",
  "职场人的成熟，是听到再优化一下还能点头。",
  "这杯咖啡像你的电量，显示还有，但不敢乱用。",
  "你不是没睡醒，是醒了以后不太愿意承认。",
  "咖啡不是爱好，是面对工位的入场券。",
  "这杯下去，继续把突发状况处理成日常。",
  "今天的你像被压缩包，里面装满了没解开的压力。",
  "你不是太敏感，是现实的噪音太大。",
  "咖啡让人清醒，清醒让人想请年假。",
  "这杯饮品像简历，精致包装，内里辛酸。",
  "你今天最大的进步，是还没有把真实想法说出口。",
  "喝完这杯，继续用专业语气描述离谱现象。",
  "职场最微妙的地方，是明知道没用还要认真做。",
  "这杯咖啡像需求评审，刚入口就知道后面不简单。",
  "你不是在浪费时间，是时间被会议合法征用。",
  "咖啡因到岗了，快乐还在远程办公。",
  "这杯饮品提醒你，成年人没有暂停键，只有续杯键。",
  "你今天的心累，已经具备独立完成周报的能力。",
  "喝下这杯，继续把想吐槽管理成项目风险。",
  "职场最现实的愿望，是明天少一点会。",
  "这杯咖啡像下班前的消息，短短一行，毁掉一晚。",
  "你不是不想配合，是配合太多以后开始怀疑需求来源。",
  "咖啡的回甘不稳定，工作的回旋镖很稳定。",
  "这杯像工作群公告，没喝也知道会苦。",
  "你今天没有倒下，是因为系统还没允许退出。",
  "喝完以后，继续在人群里保持像素级体面。",
  "职场最难的是，不仅要解决问题，还要显得问题本来就好解决。",
  "这杯饮品像试用期，入口谨慎，后劲忐忑。",
  "你不是没有安排，你只是把真正想做的事折叠进了周末。",
  "咖啡帮你开机，现实负责加载插件。",
  "这杯下去，继续把今天过成兼容模式。",
  "今天的你像共享日历，被安排得明明白白。",
  "你不是缺安全感，是需求总在变更。",
  "咖啡不负责开心，它只负责让你别睡着。",
  "这杯咖啡像年底预算，越到后面越不够用。",
  "你今天的从容，来自对失控的熟练。",
  "喝完这杯，继续把算了说成没关系。",
  "职场最毒的鸡汤，是苦尽甘来，但没人说苦要多久。",
  "这杯饮品很有边界感，喝完就结束，不像工作。",
  "你不是被咖啡唤醒，是被未完成事项吓醒。",
  "咖啡杯很轻，拿着它的人很重。",
  "这杯像复盘会，越品越想开小号。",
  "你今天的愿望很小，电脑别卡，群别响。",
  "喝下去以后，你会更坚定地想放假。",
  "职场最常见的魔法，是把你的问题变成我的问题。",
  "这杯咖啡像团队协作，大家都在搅拌，没人知道最后什么味。",
  "你不是不够卷，是已经卷到边缘开始掉渣。",
  "咖啡能续身体，不能续对工作的好奇心。",
  "这杯饮品没有未来感，但有下班前的求生感。",
  "你今天没有失控，只是语气里少了一点客服感。",
  "喝完这杯，继续在不想回和必须回之间选择措辞。",
  "职场人的安全感，是重要文件真的保存了。",
  "这杯咖啡很像现实，冷了以后更难入口。",
  "你不是没价值，是价值经常被开会消耗。",
  "咖啡让你暂时发光，工位让你长期放电。",
  "这杯下去，继续把被催促当作背景音乐。",
  "今天的你像工单，流转很久，没人真正解决。",
  "你不是想摆烂，是想把自己从混乱里捞出来。",
  "咖啡的苦有标准，工作的苦很个性化。",
  "这杯饮品像组织变动，入口突然，后味不明。",
  "你今天的愿望，是待办列表不要自己繁殖。",
  "喝完以后，继续把没空发疯当作自律。",
  "职场最真实的成长，是从我不会到这也归我。",
  "这杯咖啡像通勤路，知道会堵，但还是要走。",
  "你不是在找咖啡，你是在找一个还能继续的理由。",
];

const playfulQuoteGroups = [
  {
    starts: [
      "今天的咖啡像 Jira 工单",
      "今天的咖啡像飞书红点",
      "今天的咖啡像周会纪要",
      "今天的咖啡像共享文档",
      "今天的咖啡像排期表",
      "今天的咖啡像老板的语音",
      "今天的咖啡像需求评审",
      "今天的咖啡像日报模板",
      "今天的咖啡像审批流",
      "今天的咖啡像项目群",
      "今天的咖啡像会议邀请",
      "今天的咖啡像线上 bug",
    ],
    ends: [
      "看着有人负责，点开全是待确认",
      "一开始很简单，后面越喝越不像人话",
      "表面很短，实际能占用你一整个下午",
      "每个人都路过，最后默认你来收尾",
      "越往下看越清醒，越清醒越想静音",
      "入口是风味，后劲是改到第七版",
      "闻起来很正常，喝完发现只是排期提前",
      "名字很专业，内容主要靠猜",
      "打开之前还行，打开之后血压很行",
      "不是不能做，是做完就会长出三个新问题",
    ],
  },
  {
    starts: [
      "你现在的状态像冰美式",
      "你现在的状态像冷萃",
      "你现在的状态像加浓美式",
      "你现在的状态像燕麦拿铁",
      "你现在的状态像生椰拿铁",
      "你现在的状态像焦糖玛奇朵",
      "你现在的状态像Dirty",
      "你现在的状态像气泡美式",
      "你现在的状态像话梅美式",
      "你现在的状态像手冲咖啡",
      "你现在的状态像速溶咖啡",
      "你现在的状态像办公室咖啡机",
    ],
    ends: [
      "外表很稳，内核全靠咖啡因托管",
      "看起来高级，实际上只想下班",
      "入口冷静，后劲全是红点提醒",
      "香是香的，人是被迫营业的",
      "甜度刚好，工作量不太刚好",
      "第一口是体面，第二口是算了",
      "杯子很精致，心态很散装",
      "表面有层次，后台全是报警",
      "名字很漂亮，喝完还是要改 PPT",
      "看着像生活方式，其实是上班方式",
    ],
  },
  {
    starts: [
      "喝完这杯",
      "拍完这杯",
      "录入图鉴以后",
      "打卡成功以后",
      "咖啡因上线以后",
      "杯盖扣上以后",
      "冰块融化之前",
      "第一口下去以后",
      "咖啡见底以后",
      "今日份续航启动后",
      "精神状态重启后",
      "手机拍照识别成功后",
    ],
    ends: [
      "继续把离谱需求翻译成友好表达",
      "继续把想下班包装成我再看看",
      "继续把无语改写成收到",
      "继续把不想开会伪装成网络不稳",
      "继续把心态崩了压缩成一个句号",
      "继续把这个不归我管说得很委婉",
      "继续把想拒绝改成我评估一下",
      "继续把没空处理成排期较满",
      "继续把锅从天上接到待办里",
      "继续把今天演成一个稳定版本",
    ],
  },
  {
    starts: [
      "早会前这杯",
      "午饭后这杯",
      "下午三点这杯",
      "下班前这杯",
      "改稿前这杯",
      "评审前这杯",
      "复盘前这杯",
      "周报前这杯",
      "对齐前这杯",
      "同步前这杯",
      "催进度前这杯",
      "被拉会前这杯",
    ],
    ends: [
      "不是咖啡，是表情管理预付款",
      "不是饮品，是职场语气缓冲垫",
      "不是享受，是成年人开机仪式",
      "不是仪式感，是不发疯的必要成本",
      "不是提神，是给礼貌续费",
      "不是上瘾，是工作流程的外接电源",
      "不是奖励，是避免把真话说出口",
      "不是生活品质，是工位求生道具",
      "不是精致，是今天还没倒下的证据",
      "不是浪漫，是对消息提示音的防御",
    ],
  },
  {
    starts: [
      "你的大脑像测试环境",
      "你的耐心像免费试用",
      "你的日程像压缩包",
      "你的工位像充电桩",
      "你的待办像无限滚动列表",
      "你的情绪像后台任务",
      "你的电脑像老员工",
      "你的工资像加载动画",
      "你的下班计划像产品路线图",
      "你的周末像限量优惠券",
      "你的午休像临时文件",
      "你的咖啡杯像项目经理",
    ],
    ends: [
      "能跑，但没人敢保证稳定",
      "显示还有，其实马上到期",
      "解开一个，里面还有三个",
      "大家都来蹭电，没人问你烫不烫",
      "滑不到底，也不敢刷新",
      "一直运行，但不能展示给客户看",
      "反应慢一点，但真的承受很多",
      "转了很久，金额依旧克制",
      "写得很美，落地时间待定",
      "看起来存在，真正用到很难",
    ],
  },
  {
    starts: [
      "这杯咖啡给你的不是能量",
      "这杯咖啡给你的不是勇猛",
      "这杯咖啡给你的不是灵感",
      "这杯咖啡给你的不是答案",
      "这杯咖啡给你的不是好运",
      "这杯咖啡给你的不是效率",
      "这杯咖啡给你的不是从容",
      "这杯咖啡给你的不是底气",
      "这杯咖啡给你的不是奇迹",
      "这杯咖啡给你的不是松弛",
      "这杯咖啡给你的不是快乐",
      "这杯咖啡给你的不是自由",
    ],
    ends: [
      "是看完需求还能坐稳的基本素养",
      "是面对红点仍然不摔手机的自控力",
      "是把脏话自动替换成好的收到的能力",
      "是看到排期提前还能保持微笑的技术",
      "是让你在会议里看起来仍有电的滤镜",
      "是把低电量强行伪装成高性能模式",
      "是让你更清楚地知道哪里不对劲",
      "是把今天从想下班边缘拖回工位边缘",
      "是短暂相信自己还能抢救一下的幻觉",
      "是让你手速变快，心态不一定变好",
    ],
  },
  {
    starts: [
      "如果这杯咖啡会说话",
      "如果工位会说话",
      "如果会议室会说话",
      "如果需求文档会说话",
      "如果周报会说话",
      "如果打卡机会说话",
      "如果电脑风扇会说话",
      "如果消息红点会说话",
      "如果外卖咖啡会说话",
      "如果杯底会说话",
      "如果键盘会说话",
      "如果日历会说话",
    ],
    ends: [
      "它大概会先劝你保存文件",
      "它大概会问你怎么又来了",
      "它大概会说别装了你根本没听懂",
      "它大概会提醒你这锅有点烫",
      "它大概会建议你先别点开群聊",
      "它大概会把你加入班味白名单",
      "它大概会说这需求不关咖啡的事",
      "它大概会问你有没有考虑静音老板消息",
      "它大概会把今日状态标记为勉强运行",
      "它大概会和你一起等下班",
    ],
  },
  {
    starts: [
      "这杯的前调是",
      "这杯的中调是",
      "这杯的后调是",
      "这杯的余韵是",
      "这杯的隐藏风味是",
      "这杯的职场风味是",
      "这杯的办公风味是",
      "这杯的会议风味是",
      "这杯的加班风味是",
      "这杯的打工风味是",
      "这杯的红点风味是",
      "这杯的绩效风味是",
    ],
    ends: [
      "赶地铁，赶进度，赶紧别找我",
      "低电量，高压力，中度想请假",
      "表面丝滑，底部有锅",
      "先香一下，再慌一会儿",
      "需求变更，排期压缩，老板很急",
      "微苦，微酸，微微想关机",
      "咖啡因明显，班味也明显",
      "入口清醒，回味想逃",
      "像努力过，但没完全努力动",
      "像能撑住，但不能保证多久",
    ],
  },
  {
    starts: [
      "今天的你适合搭配",
      "今天的会议适合搭配",
      "今天的周报适合搭配",
      "今天的需求适合搭配",
      "今天的排期适合搭配",
      "今天的老板消息适合搭配",
      "今天的项目群适合搭配",
      "今天的工位适合搭配",
      "今天的电脑适合搭配",
      "今天的待办事项适合搭配",
      "今天的同步会适合搭配",
      "今天的复盘会适合搭配",
    ],
    ends: [
      "一杯冰美式和三次深呼吸",
      "一杯拿铁和一个专业假笑",
      "一杯冷萃和一份不打开的聊天框",
      "一杯 Dirty 和一句我确认一下",
      "一杯燕麦拿铁和一颗不追问的心",
      "一杯加浓和一个别找我的眼神",
      "一杯热美式和一套表情管理",
      "一杯生椰拿铁和一个静音按钮",
      "一杯手冲和一份装作听懂的姿态",
      "一杯摩卡和一张别催了的脸",
    ],
  },
  {
    starts: [
      "咖啡因刚上线",
      "咖啡杯刚见底",
      "咖啡香刚散开",
      "咖啡还没喝完",
      "咖啡刚送到",
      "咖啡还很烫",
      "咖啡已经冷了",
      "咖啡刚被扫码",
      "咖啡刚录入图鉴",
      "咖啡刚完成认证",
      "咖啡刚进入今日库存",
      "咖啡刚结束拍照环节",
    ],
    ends: [
      "新需求就开始敲门，特别懂时间管理",
      "会议邀请就发来了，像怕你过得太安静",
      "老板消息就到了，连香气都来不及撤退",
      "工作群就亮了，仿佛闻到了你的清醒",
      "排期就提前了，像专门等它上岗",
      "同事就问在吗，问得很轻，活很重",
      "待办就新增了，咖啡杯当场装没看见",
      "文档就改版了，版本号都笑出了声",
      "bug 就复现了，仿佛在庆祝你醒来",
      "周报就提醒了，真是一点空档都不浪费",
    ],
  },
  {
    starts: [
      "有些会议像脱脂牛奶",
      "有些周报像无糖可乐",
      "有些需求像盲盒",
      "有些排期像魔术",
      "有些老板消息像惊悚片预告",
      "有些同步会像广播体操",
      "有些复盘像法庭辩论",
      "有些 PPT 像精装修毛坯",
      "有些日报像天气预报",
      "有些群公告像闹钟",
      "有些 OKR 像星座运势",
      "有些工作流像迷宫",
    ],
    ends: [
      "名字听着健康，喝完只剩空虚",
      "看起来很清爽，实际一点甜头没有",
      "拆之前期待，拆之后想退货",
      "上一秒很宽松，下一秒凭空消失",
      "还没点开，手心已经开始出汗",
      "大家动作整齐，但没人知道为什么",
      "说是找原因，最后都在找背锅的人",
      "远看很高级，近看全靠复制粘贴",
      "每天都报，准不准另说",
      "本来能睡，非要把你叫醒",
    ],
  },
  {
    starts: [
      "咖啡和工资最大的区别是",
      "咖啡和年终奖最大的区别是",
      "咖啡和绩效最大的区别是",
      "咖啡和晋升最大的区别是",
      "咖啡和老板承诺最大的区别是",
      "咖啡和团建最大的区别是",
      "咖啡和周末最大的区别是",
      "咖啡和请假最大的区别是",
      "咖啡和加班费最大的区别是",
      "咖啡和预算最大的区别是",
      "咖啡和调休最大的区别是",
      "咖啡和表扬最大的区别是",
    ],
    ends: [
      "咖啡真的会到账",
      "咖啡不用等到年底才知道有没有",
      "咖啡苦得比较直接",
      "咖啡至少有明确容量",
      "咖啡不会说下个季度再看",
      "咖啡不会占用你的休息日",
      "咖啡今天就能拥有",
      "咖啡不用写理由",
      "咖啡不是传说中的东西",
      "咖啡少了你能看出来",
      "咖啡喝完不会被追回",
      "咖啡不会只出现在大群里",
    ],
  },
  {
    starts: [
      "所谓职场成熟",
      "所谓高效协作",
      "所谓向上管理",
      "所谓结果导向",
      "所谓快速响应",
      "所谓长期主义",
      "所谓主动担当",
      "所谓闭环意识",
      "所谓充分沟通",
      "所谓战略对齐",
      "所谓轻松一下",
      "所谓简单同步",
    ],
    ends: [
      "就是把想翻白眼练成点头",
      "就是让一个人写完，大家一起署名",
      "就是提前猜到老板今天想换哪张饼",
      "就是过程全是坑，结果还要好看",
      "就是消息刚响，你的血压先响应",
      "就是这个月的活，下个月继续背",
      "就是锅还没落地，你已经伸手了",
      "就是事情没完，人先闭环",
      "就是同一件事换三种说法再讲一遍",
      "就是大家都说懂了，然后私下再问你",
      "就是换个房间继续上班",
      "就是从半小时开始，按一小时结算",
    ],
  },
  {
    starts: [
      "这杯像领导说的辛苦了",
      "这杯像同事说的帮个小忙",
      "这杯像老板说的简单改改",
      "这杯像甲方说的感觉不对",
      "这杯像产品说的先做个 MVP",
      "这杯像运营说的蹭个热点",
      "这杯像财务说的流程不对",
      "这杯像 HR 说的我们聊聊",
      "这杯像客服说的用户反馈很多",
      "这杯像法务说的有点风险",
      "这杯像技术说的理论上可以",
      "这杯像设计说的差点感觉",
    ],
    ends: [
      "前面四个字很暖，后面通常还有任务",
      "小忙不小，咖啡先苦为敬",
      "改的是细节，动的是地基",
      "感觉很玄，工期很实",
      "MVP 很轻，背锅很重",
      "热点很热，你的下班很冷",
      "流程像迷宫，你像小白鼠",
      "语气很温柔，内容通常不简单",
      "反馈很多，具体全靠你猜",
      "风险有点，咖啡多点",
      "理论上可以，实践上别问",
      "感觉差点，今晚差点下班",
    ],
  },
  {
    starts: [
      "如果把上班做成综艺",
      "如果把周会做成综艺",
      "如果把汇报做成综艺",
      "如果把改稿做成综艺",
      "如果把绩效面谈做成综艺",
      "如果把项目复盘做成综艺",
      "如果把抢会议室做成综艺",
      "如果把请假审批做成综艺",
      "如果把下班前十分钟做成综艺",
      "如果把外卖咖啡做成综艺",
      "如果把办公室聊天做成综艺",
      "如果把工位生存做成综艺",
    ],
    ends: [
      "淘汰机制就是谁先说真话谁先输",
      "观众投票环节会被取消，因为大家都想取消会议",
      "高光时刻是投屏成功",
      "反转点是终版后面还有终终版",
      "导师点评是你还可以再主动一点",
      "悬念是最后锅会落到谁手里",
      "冠军奖励是一个更小的会议室",
      "隐藏关卡是理由写得像论文",
      "最刺激，因为任何一句在吗都可能改写结局",
      "广告语是半小时送达，一天内续命",
      "名场面是所有人同时说我没意见",
      "最终任务是撑到下班并保持人形",
    ],
  },
  {
    starts: [
      "今天这杯适合发朋友圈",
      "今天这杯适合发工作群",
      "今天这杯适合发小号",
      "今天这杯适合发给老板",
      "今天这杯适合发给同事",
      "今天这杯适合发给甲方",
      "今天这杯适合发给 HR",
      "今天这杯适合发给财务",
      "今天这杯适合发给自己",
      "今天这杯适合发给闹钟",
      "今天这杯适合发给日历",
      "今天这杯适合发给空气",
    ],
    ends: [
      "配文是精致生活，实际是保命道具",
      "配文是已收到，实际是已麻木",
      "配文是别问，问就是还活着",
      "配文是我再看看，实际是我想回家",
      "配文是辛苦大家，实际是辛苦我自己",
      "配文是我们再优化，实际是我先冷静",
      "配文是状态稳定，实际是咖啡因托管",
      "配文是流程已走，实际是人也快走了",
      "配文是撑住，实际是先别打开群",
      "配文是你再响一个试试",
      "配文是今天能不能少塞点",
      "配文是没人懂也挺好",
    ],
  },
  {
    starts: [
      "咖啡是办公室的临时演员",
      "PPT 是办公室的特效师",
      "周报是办公室的旁白",
      "会议是办公室的长镜头",
      "老板是办公室的编剧",
      "需求是办公室的反派",
      "排期是办公室的悬疑线",
      "绩效是办公室的隐藏结局",
      "工位是办公室的主舞台",
      "工作群是办公室的弹幕",
      "下班是办公室的彩蛋",
      "调休是办公室的都市传说",
    ],
    ends: [
      "戏份不多，但每场都得出现",
      "内容不够，动画来凑",
      "没人爱听，但每天更新",
      "剪不掉，也快进不了",
      "随时能让剧情大改",
      "一出场就带来转折",
      "看似清楚，实际全是伏笔",
      "不到最后一集没人敢猜",
      "你每天都在这里完成角色扮演",
      "刷得越快，人越安静",
      "出现很晚，但值得等",
      "听过很多次，见过很少次",
    ],
  },
  {
    starts: [
      "脱口秀里讲职场",
      "办公室里讲幽默",
      "茶水间里讲理想",
      "电梯里讲近况",
      "会议室里讲效率",
      "工位上讲松弛",
      "周报里讲成果",
      "日报里讲进展",
      "绩效里讲成长",
      "团建里讲快乐",
      "年会里讲未来",
      "下班路上讲复盘",
    ],
    ends: [
      "最难的是不能把真实姓名说出来",
      "最大的问题是大家都笑不太出来",
      "容易被咖啡机当场打断",
      "通常从还行开始，以挺忙结束",
      "听起来像段子，坐进去像连续剧",
      "听起来很时髦，执行起来很加班",
      "成果很多，主要是活了下来",
      "进展稳定，稳定地没完没了",
      "成长明显，主要是脾气变隐形了",
      "快乐存在，通常在通知外",
      "未来很大，眼下很满",
      "复盘结果是明天还得来",
    ],
  },
] as const;

const shareableCoffeeQuotes = [
  "今日咖啡打卡：不是热爱生活，是生活需要我先开机。",
  "今天这杯的功能很明确：防止本人在会议里露出真实表情。",
  "咖啡已到账，情绪请稍后，工资请加急。",
  "今日份续航完成，接下来进入礼貌营业模式。",
  "这杯不是咖啡，是我和工作之间的缓冲垫。",
  "喝咖啡不是为了优雅，是为了把白眼憋成微笑。",
  "今日状态：人还在，心已切到勿扰模式。",
  "这杯下去，继续把不想上班翻译成精神饱满。",
  "咖啡因让我清醒，工作群让我后悔清醒。",
  "今天的我不需要鼓励，只需要少一个红点。",
  "这杯的含义是：别问，问就是还能撑。",
  "成年人喝咖啡，喝的不是风味，是重新做人倒计时。",
  "今日工位限定饮品：一口咖啡，一口我再看看。",
  "喝完这杯，继续做朋友圈里精致、工位上卡顿的人。",
  "这杯咖啡很懂分寸，至少不会问我进展怎么样。",
  "今天的班味太浓，只能用咖啡压一压。",
  "咖啡是我的外接电源，老板消息是我的强制重启。",
  "这杯不是提神，是把想走的我按回座位。",
  "打卡成功：今日份清醒已充值，快乐不在套餐内。",
  "喝完这杯，继续把心里那句凭什么改成好的。",
  "咖啡杯见底了，我的耐心也正在走结算流程。",
  "今天不想做职场精英，只想做一杯冰美式的临时用户。",
  "这杯咖啡的名字应该叫：请勿打扰本人正在营业。",
  "咖啡因上岗，理智返场，真实想法暂不公开。",
  "今日目标很朴素：不把心里话发进工作群。",
  "一杯咖啡解决不了问题，但能让我更体面地面对问题。",
  "这杯的风味是：前调开机，中调开会，后调开摆失败。",
  "咖啡提醒我：人可以苦，但文案要好笑。",
  "今天的我像外卖咖啡，外包装完整，里面晃得厉害。",
  "这杯咖啡不治本，但能把本人暂时设置为可用。",
  "朋友圈看我在喝咖啡，公司看我在续航。",
  "今日份体面，由咖啡因和职业假笑联合出品。",
  "这杯是成年人版快乐水：喝完不一定快乐，但至少醒着。",
  "咖啡让我醒来，排期让我清醒过头。",
  "本人今日运行环境：咖啡因 80%，工资条 20%。",
  "这杯咖啡的作用，是把离职冲动改成下班再说。",
  "喝咖啡前：我不行了。喝咖啡后：我可以再装一下。",
  "今天的我没有发疯，是因为咖啡还没喝完。",
  "这杯咖啡是我的职场滤镜，滤掉了三分真实表情。",
  "开会前喝咖啡，是对自己最后的温柔，也是对同事的保护。",
  "今日工位喜剧：本人上线，灵感离线，消息不断线。",
  "咖啡不贵，贵的是喝完还要继续上班。",
  "这杯下肚，继续在已读和已懂之间反复横跳。",
  "今天不是想喝咖啡，是想拥有一种我还行的证据。",
  "咖啡让我短暂相信，今天能用成年人的方式处理荒唐。",
  "这杯很适合截图，因为工作本身不太适合截图。",
  "今日打卡：把上班过成副本，把咖啡喝成装备。",
  "咖啡是道具，工位是地图，红点是野怪刷新提示。",
  "这杯的隐含意思是：本人已开机，请勿过度使用。",
  "咖啡因不是万能的，但没有咖啡因是万万不能开早会的。",
  "今日心态：能解决就解决，不能解决就先喝一口。",
  "这杯咖啡像我的职业规划：看起来很浓，实际随时会凉。",
  "喝完这杯，继续把我不理解包装成我确认一下。",
  "今天的咖啡很香，工位很稳，我很想走。",
  "咖啡杯很小，装不下工作群里那些大动静。",
  "今日份清醒提醒：你不是爱咖啡，你是怕开会时睡着。",
  "这杯咖啡的后劲，是让我更准确地知道哪里离谱。",
  "上班像长跑，咖啡像路边补给，终点叫下班。",
  "喝咖啡是我的个人仪式，点开群聊是我的工伤现场。",
  "这杯咖啡没有人情世故，苦得很坦荡。",
  "今日人设：精致喝咖啡，狼狈赶需求。",
  "咖啡让我保持清醒，清醒让我想改喝奶茶。",
  "这杯下去，继续把普通一天演成职场真人秀。",
  "今天的我和咖啡一样：被摇匀后继续出售。",
  "咖啡是短暂回血，会议是即时扣血。",
  "这杯咖啡建议配合静音模式食用。",
  "今日份小乐子：咖啡比工作更像工作，至少准时到。",
  "喝完这杯，我可以更平静地说出这不合理。",
  "咖啡不是逃避，是暂停三分钟再回去面对。",
  "这杯的价值在于，它不会突然说我们简单同步一下。",
  "今日状态：上班可以，别突然挑战我的表情管理。",
  "咖啡把我叫醒，消息把我叫老。",
  "这杯咖啡像工作群截图，不发出来憋得慌。",
  "喝咖啡是为了把今天从离谱模式调成可运行模式。",
  "今日打工玄学：先喝咖啡，再相信自己。",
  "这杯咖啡很适合我的班味，苦得刚好，贵得清醒。",
  "朋友圈负责精致，咖啡负责续命，我负责别穿帮。",
  "这杯喝完，继续假装下班这件事仍然有确定性。",
  "咖啡让我有精神，但没有让我有多余的感情。",
  "今天这杯属于职场防晒：防止本人被需求晒干。",
  "这杯咖啡是我的工位小短剧，剧情叫强撑但好笑。",
  "喝完以后，继续把老板的临时想法当限时活动处理。",
  "今日咖啡文案：生活不一定有趣，但我可以先把它说得有趣。",
  "这杯不是奖励，是成年人给自己的临时补偿。",
  "咖啡越喝越清醒，越清醒越想给工作群起个别名。",
  "今天的我适合低温保存，避免被需求加热。",
  "一杯咖啡下去，班没少上，但段子多了一个。",
  "咖啡杯举起来的那一刻，我和今天短暂和解三秒。",
  "这杯咖啡建议命名为：先别找我，我正在加载。",
  "今日份幽默感充值成功，使用场景：被安排新活时。",
  "喝咖啡最大的意义，是让我有力气把无语说得很礼貌。",
  "这杯咖啡的含金量，是我没有立刻关电脑。",
  "今天也在用咖啡，把普通上班伪装成都市生活方式。",
  "咖啡喝完前，我暂时相信一切还能协商。",
  "这杯的朋友圈定位：公司附近，精神边缘。",
  "打工人的咖啡不是饮品，是和世界继续周旋的许可证。",
  "今日咖啡打卡：我不是醒了，我是被任务点亮了。",
  "这杯让我明白，人类发明咖啡可能就是为了开会。",
  "喝完这杯，继续以优雅姿态处理不优雅的需求。",
  "咖啡把我从困里捞出来，又把我送回工位里。",
  "今天这杯的口感：一点香，一点苦，很多班。",
  "这杯咖啡像我对工作的态度：能喝，但别细品。",
  "今日份反讽：我在认真记录咖啡，顺便记录自己没倒下。",
  "咖啡打卡成功，今日职场荒诞喜剧正式开演。",
  "这杯适合发朋友圈，配文：本人正在进行低成本自救。",
  "喝咖啡不是精致，是我对工位发起的温和反击。",
  "今日状态很简单：咖啡在手，废话免谈。",
  "这杯下去，继续把工作群当大型沉浸式剧本杀。",
  "咖啡让我重新上线，但不代表我同意被无限调用。",
  "今天的我不是没有热情，是热情正在走审批。",
  "这杯咖啡是我的小型下班预告，虽然预告片很长。",
  "喝完这杯，我又可以用温柔语气处理荒唐请求了。",
  "咖啡不是生活方式，是职场求生方式穿了个好看的杯套。",
  "今日份截图理由：这杯咖啡比今天的进展更有成就感。",
  "这杯咖啡负责提神，我负责把它讲成段子。",
];

void baseToxicQuotes;
void playfulQuoteGroups;
void shareableCoffeeQuotes;

// Only hand-reviewed lines are served to users. The older source collections stay
// above for editorial reference, but generated start/end combinations are no longer
// built at runtime because too many of them were repetitive or semantically loose.
const handReviewedToxicQuotes = [
  "领导说辛苦了，通常是下一项任务的片头。",
  "“简单改改”的简单，主要体现在说这句话很简单。",
  "这个需求唯一稳定的部分，是它一直在变。",
  "会议很成功，成功地让所有人晚下班半小时。",
  "今天没有无效沟通，因为沟通也没发生。",
  "大家都对齐了，除了目标、时间和理解。",
  "所谓快速响应，就是消息刚响，血压先到。",
  "方案已经很成熟了，成熟到可以推倒重来。",
  "终版的意义，是提醒你准备建立终版二。",
  "项目进展顺利，顺利地进入了下一轮延期。",
  "老板画的饼很圆，主要因为没有落地的棱角。",
  "今天也在降本增效：降低快乐，增加效率。",
  "工作很有价值，尤其是写周报描述它的时候。",
  "需求没有插队，它只是从天花板掉了下来。",
  "会议纪要很短：问题还在，时间没了。",
  "这不是加班，是下班时间的弹性办公。",
  "公司给了我成长空间，主要用来装新任务。",
  "我的职业规划很清晰：先把今天混过去。",
  "绩效沟通很温暖，饼是刚出炉的。",
  "晋升通道一直畅通，只是入口暂未开放。",
  "工资到账像系统通知，点开前总有一点期待。",
  "年终奖不是没有，只是采用了无实物表演。",
  "调休是职场里的薛定谔假期，不休时它存在。",
  "团建的核心，是在休息日讨论团队为什么不够团结。",
  "带薪摸鱼最大的风险，是鱼突然开会。",
  "工位很稳定，稳定地看着我失去稳定。",
  "打卡证明我来过，产出证明我没白来太久。",
  "工作群没有恶意，它只是二十四小时都有想法。",
  "消息已读，灵魂未必在线。",
  "收到，是成年人最简短的情绪管理。",
  "好的，翻译一下就是我先忍住。",
  "我再确认一下，通常是礼貌版的你认真的吗。",
  "稍后同步，职场里最温柔的恐怖预告。",
  "有空吗，是每个临时任务的开场白。",
  "帮个小忙，小到刚好占满今天。",
  "你先出一版，意思是大家先看看怎么推翻。",
  "先做个 MVP，后来 M 没了，只剩 VP。",
  "这个事情不复杂，复杂的是为什么轮到我。",
  "理论上可以，是技术同事最后的善意。",
  "感觉不对，是甲方最稳定的设计规范。",
  "再优化一下，是没有终点的委婉说法。",
  "复盘不是为了追责，是为了准确找到负责的人。",
  "问题已经闭环了，人还在环里。",
  "沟通充分到每个人都保留了自己的理解。",
  "我们尊重专业，所以每个人都提了修改意见。",
  "数据会说话，只是大家想听的方言不同。",
  "结果导向，就是过程的坑最后都要长成结果。",
  "长期主义，就是这月没做完，下月继续做。",
  "主动担当，就是锅还在空中，手已经伸出去了。",
  "向上管理，就是提前猜老板今天想吃哪张饼。",
  "扁平化管理，让每一口锅都能垂直落下。",
  "弹性工作制，弹的是下班时间。",
  "灵活办公，主要是你要灵活，公司不用。",
  "公司像家，所以偶尔也让你住公司。",
  "氛围很好，好到没人敢先下班。",
  "同事都很年轻，因为成熟的人已经不问为什么。",
  "团队执行力很强，尤其是执行临时需求。",
  "我们没有内卷，只是大家碰巧都不敢停。",
  "这份工作很自由，自由地接受所有安排。",
  "今天准时下班的计划，完成了计划部分。",
  "下班不是结束，是工作群换了个背景。",
  "周末很充实，消息把空白都填满了。",
  "假期的意义，是换个地方担心工作。",
  "闹钟负责叫醒我，工资负责说服我。",
  "早八不可怕，可怕的是早八还有会。",
  "午休很完整，完整地用来回复上午的消息。",
  "晚上睡不着，可能是白天的需求还没下班。",
  "情绪稳定，是因为情绪正在走审批。",
  "精神饱满，是职场妆容的一部分。",
  "松弛感有了，主要是进度已经救不回来了。",
  "今天心态很好，好就好在还没打开群。",
  "本人状态稳定，稳定在想下班。",
  "成年人的体面，是把白眼压缩成一个微笑。",
  "脾气变好了，因为表达欲被排期占满了。",
  "不是没有意见，是意见暂不支持公开访问。",
  "我没有摆烂，只是切换到了节能模式。",
  "人还在线，热情正在后台更新。",
  "工作教会我成长，也教会我清空聊天记录。",
  "今天的成就，是没把真话发进工作群。",
  "咖啡因上岗，真实想法继续休假。",
  "咖啡让我清醒，清醒让我看得更离谱。",
  "这杯不是提神，是把离职冲动改成下班再说。",
  "咖啡不解决问题，只提高我发现问题的清晰度。",
  "咖啡和工资的区别是，咖啡真的会到账。",
  "咖啡苦得坦荡，工作苦得需要写总结。",
  "一杯咖啡解决不了问题，但能延迟我的真实反应。",
  "喝咖啡前我不行了，喝完我可以再装一会儿。",
  "咖啡杯见底了，耐心也在走结算流程。",
  "咖啡是外接电源，工作群是强制重启。",
  "这杯的风味：前调开机，中调开会，后调想走。",
  "今天的班味太浓，只能拿咖啡压一压。",
  "咖啡让我重新上线，不代表可以无限调用。",
  "开会前喝咖啡，是对自己和同事的双重保护。",
  "这杯咖啡不会问进展，已经赢过很多同事了。",
  "咖啡越喝越醒，需求越看越像梦。",
  "这杯不贵，贵的是喝完还得继续上班。",
  "咖啡负责续命，我负责把续命写成生活方式。",
  "今日份体面，由咖啡因和职业假笑联合出品。",
  "咖啡先凉没关系，需求千万别再热起来。",
  "杯里是咖啡，眼里是下班，手里是新需求。",
  "这杯喝完，继续把荒唐回复成好的收到。",
  "咖啡没有人情世故，苦得非常直接。",
  "老板说别有压力，压力听完很有压力。",
  "工作没有背叛我，它从一开始就没承诺快乐。",
  "今天效率很高，高效完成了接受新任务。",
  "方案没有被否定，只是优点暂时没被发现。",
  "大家畅所欲言，然后按领导的意见执行。",
  "这次沟通很坦诚，除了真正想说的话。",
  "领导很信任我，所以什么都让我自己想办法。",
  "项目没有延期，只是交付日期获得了自由。",
  "我热爱挑战，尤其是挑战准时下班。",
  "公司鼓励创新，所以需求每天都不一样。",
  "今日工作完成度：情绪处理百分之百。",
  "会议需要我，主要需要我的在线状态。",
  "任务很有成长性，昨天一项，今天三项。",
  "职业发展很快，待办增长得尤其快。",
  "办公室最短的路，是从工位走到新任务。",
  "今天没有摸鱼，是鱼主动游进了会议室。",
  "我和下班双向奔赴，工作负责制造距离。",
  "一切尽在掌握，掌握的人暂时不在群里。",
  "同事说马上，时间说你们聊。",
  "这份工作很稳定，稳定地每天给我惊喜。",
  "今天适合努力，明天适合解释今天的努力。",
  "问题不大，大的是解决问题的人。",
  "老板很随和，随时都能和新需求一起出现。",
  "计划赶不上变化，变化正在赶下一个计划。",
  "工作使我充实，主要充实了日历。",
] as const;

export const toxicQuotes = [
  ...handReviewedToxicQuotes,
  ...buildQualityControlledToxicQuotes(),
];

export function getCaffeine(coffeeType: string, volumeMl: number) {
  return Math.round(((caffeinePer100ml[coffeeType] ?? 30) * volumeMl) / 100);
}

export function getRandomToxicQuote(seed = Date.now()) {
  return toxicQuotes[Math.abs(seed) % toxicQuotes.length];
}

export function getNextToxicQuote(current: string, seed = Date.now()) {
  const candidateIndex = Math.abs(seed) % toxicQuotes.length;
  const currentIndex = toxicQuotes.indexOf(current);
  const nextIndex = candidateIndex === currentIndex
    ? (candidateIndex + 1) % toxicQuotes.length
    : candidateIndex;

  return toxicQuotes[nextIndex];
}
