...
- 手机端先保存记录并立即显示结果卡，再在后台抠除背景、生成主体轮廓贴纸并回填日历；原始照片与贴纸图分别保留。
- PC 端支持图鉴浏览、统计、日志、详情、删除。
- PC 与手机端均支持按月查看咖啡日历；每天展示当天最新一杯贴纸，点击日期后并列展示当天全部记录，再点击单条记录进入详情。
...
- 上传或拍摄咖啡照片。
- 压缩图片为 JPEG data URL。
- 记录提交不等待贴纸，保存成功后立即显示毒鸡汤反馈卡。
- 后台通过可终止的 Web Worker 使用量化模型抠除背景，主线程只负责按主体 alpha 轮廓裁切并烘焙白色描边，避免推理卡住录入界面。
- 不支持可终止 Worker 的旧浏览器不在主线程执行模型；记录照常保存，并由之后打开的兼容手机或电脑页面续补贴纸。
- 版本化的模型与 WASM 由应用同源托管并长期缓存，避免手机首次生成依赖境外 CDN；用户选择照片后即开始预载。
- Next 开发与生产构建固定使用 Webpack，以确保贴纸 Worker 被编译为独立 JavaScript 线程入口。
- 后台贴纸完成后按记录 ID 回填并替换日历占位；抠图失败不影响已经保存的记录。
- 手机和电脑在空闲时都会逐日补全缺失贴纸并升级旧版贴纸，交互式新照片优先处理；失败采用退避重试。
- 调用 AI 判断图片是否像饮品。
- AI 失败时进入人工确认模式。
- 搜索咖啡名称。
- 选择咖啡大类和子类。
...
- 展示手机端入口二维码。
- 小屏自动跳转 `/mobile`。
- 展示月历：日期按本地时区归组，每天显示最新记录的轮廓贴纸缩略图；生成期间以最新照片的无框预览过渡，同日多杯显示数量角标。
- 点击有记录的日期后，以图鉴卡片网格展示当天全部记录，再点击卡片查看单条详情。
...
- 8 大类。
- 76 个子类（以 `coffee-data.ts` 当前数据为准）。
...
- `stickerData`: 透明背景贴纸 data URL 或 Supabase public URL。
- `stickerVersion`: 贴纸生成算法版本；低版本记录可在客户端空闲时升级。
- `caffeine`: 咖啡因估算值。
- `temp`: 温度标签。
- `sugar`: 糖度标签。
- `aiComment`: 类型对应评语。
